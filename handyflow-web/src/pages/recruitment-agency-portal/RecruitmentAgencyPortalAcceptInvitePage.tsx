// src/pages/recruitment-agency-portal/RecruitmentAgencyPortalAcceptInvitePage.tsx
...
export function RecruitmentAgencyPortalAcceptInvitePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await recruitmentAgencyPortalApi.register({ inviteToken: token, password, fullName })
      setAuth(res.token, { portalUserId: res.portalUserId, email: res.email, fullName: res.fullName })
      navigate("/recruitment-agency/portal", { replace: true })
    } catch (err: any) {
