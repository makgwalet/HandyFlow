...
export function CreateQuotePage() {
  ...
  const validate = (): boolean => {
    ...
    if (clientType === 'existing' && !customerId) errs.customerId = 'Please select a customer'
    if (clientType === 'walkin'   && !walkinName.trim()) errs.walkinName = 'Walk-in client name is required'
    if (!title.trim()) errs.title = 'Quote title is required'
    if (lineItems.length === 0) errs.lineItems = 'Add at least one line item'
    const blankDesc = lineItems.some(l => !l.description.trim())
    if (blankDesc) errs.lineItems = 'All line items must have a description'
    setFieldErrors(errs)
