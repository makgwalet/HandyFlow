// src/pages/warehousing-portal/WhsePortalAcceptInvitePage.tsx
//
// Reads the invite token from the URL query string (?token=...) — same
// convention as CollAgencyPortalAcceptInvitePage.tsx. Posts to
// /api/v1/warehousing/portal/auth/register, confirmed via
// WhsePortalAuthController source.
...
export function WhsePortalAcceptInvitePage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await apiClient.post("/api/v1/warehousing/portal/auth/register", { inviteToken: token, password, fullName })
      ...
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 36, width: 380, boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--hf-text-faint)", margin: 0 }}>Warehousing Client Portal</p>
          </div>
        </div>
