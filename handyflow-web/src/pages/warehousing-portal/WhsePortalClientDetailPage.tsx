  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--hf-surface-sunken)", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: 860, margin: "0 auto", padding: "28px 24px" }}>
        <button onClick={() => navigate("/warehousing/portal")} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "var(--hf-text-muted)", fontSize: 13, marginBottom: 18, padding: 0 }}>
        ...
        {sub === "inventory" && (
          invLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          inventory.length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No stock on hand.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {inventory.map((inv, i) => (
                <div key={inv.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <span style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600 }}>Item {inv.itemId.slice(0, 8)}</span>
                  <span style={{ fontSize: 12.5, color: "var(--hf-text-muted)" }}>{inv.qtyOnHand} on hand · {inv.available} available</span>
                </div>
              ))}
            </div>
          )
        ...
        {sub === "inbound" && (
          inboundLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          inbound.length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No inbound shipments.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {inbound.map((s, i) => (
                <div key={s.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <span style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600 }}>{s.referenceNumber ?? `Shipment ${s.id.slice(0, 8)}`}</span>
                  <span style={{ fontSize: 11.5, color: "var(--hf-text-muted)" }}>{s.status.replace(/_/g, " ")}{s.expectedDate ? ` · Expected ${s.expectedDate}` : ""}</span>
                </div>
              ))}
            </div>
          )
        ...
        {sub === "outbound" && (
          outboundLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          outbound.length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No outbound orders.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {outbound.map((o, i) => (
                <div key={o.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <div>
                    <p style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600, margin: 0 }}>{o.orderReference ?? `Order ${o.id.slice(0, 8)}`}</p>
                    {o.shipToName && <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: 0 }}>To {o.shipToName}</p>}
                  </div>
                  <span style={{ fontSize: 11.5, color: "var(--hf-text-muted)" }}>{o.status}{o.trackingNumber ? ` · ${o.carrier ?? ""} #${o.trackingNumber}` : ""}</span>
                </div>
              ))}
            </div>
          )
