...
const inp: React.CSSProperties = { width: '100%', padding: '10px 13px', border: '1.5px solid var(--hf-border)', borderRadius: 9, fontSize: 14, boxSizing: 'border-box' as const, background: 'var(--hf-surface)', outline: 'none', fontFamily: 'inherit' }
const lbl: React.CSSProperties = { display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--hf-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 7 }
...
export function CreativeApprovePage() {
  ...
  if (isLoading) {
    return (
      <Page>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 20px', color: 'var(--hf-text-muted)' }}>
    ...
    return (
      <Page tenantName={proof?.tenantName}>
        <StatusCard icon={<CheckCircle size={48} color="#166534" />} title="Approved!" color="#166534" bg="#F0FDF4">
          ...
          <div style={{ fontSize: 13, color: 'var(--hf-text-muted)', background: 'var(--hf-surface-muted)', border: '1px solid var(--hf-border)', borderRadius: 8, padding: '10px 14px' }}>
  ...
  return (
    <Page tenantName={proof.tenantName}>
      ...
      {/* Job context header */}
      <div style={{ background: 'var(--hf-surface)', border: '1px solid var(--hf-border)', borderRadius: 14, padding: '20px 24px', marginBottom: 20, animation: 'fadeIn 0.3s ease' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--hf-text-faint)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 5 }}>
              {proof.tenantName} · Proof for review
            </div>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'var(--hf-text)', marginBottom: 5 }}>{proof.jobTitle}</h2>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--hf-warning-soft)', border: '1px solid var(--hf-warning-border)', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: 'var(--hf-warning-text)', fontWeight: 600 }}>
      ...
      {proof.approvalMode && proof.approvalMode !== 'SINGLE' && (
        <div style={{ background: 'var(--hf-info-soft)', border: '1px solid var(--hf-info-border)', borderRadius: 12, padding: '14px 18px', marginBottom: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--hf-info-text)', marginBottom: 8 }}>
      ...
      <div style={{ background: 'var(--hf-surface)', border: '1px solid var(--hf-border)', borderRadius: 14, overflow: 'hidden', marginBottom: 20, animation: 'fadeIn 0.35s ease' }}>
        ...
        <div style={{ padding: 20, minHeight: 200, background: 'var(--hf-surface-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {proof.fileUrl ? (
            fileIsImage ? (
              <div
                ref={imageWrapRef}
                  ...
                  .map((c: any, i: number) => (
                    <button
                      ...
                      style={{
                        ...
                        background: c.authorType === 'CLIENT' ? 'var(--hf-accent)' : 'var(--hf-primary)', color: 'var(--hf-text-on-solid)',
                        border: '2px solid var(--hf-surface)', boxShadow: '0 2px 6px rgba(0,0,0,0.3)',
                ...
                {pendingPin && (
                  <div style={{
                    ...
                    background: 'rgba(217,119,6,0.25)', border: '2px solid var(--hf-warning)',
              ...
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                <Package size={40} style={{ marginBottom: 12, color: 'var(--hf-text-disabled)' }} />
                <div style={{ fontWeight: 600, color: 'var(--hf-text-tertiary)', marginBottom: 8 }}>{proof.fileName}</div>
                <a
                  href={`data:${proof.fileType || 'application/octet-stream'};base64,${proof.fileUrl}`}
                  download={proof.fileName}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--hf-primary)', color: 'var(--hf-text-on-solid)', padding: '10px 20px', borderRadius: 8, textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>
                  Download to view
                </a>
              </div>
            )
            ...
            <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--hf-text-faint)' }}>
              ...
              <div style={{ fontWeight: 600, color: 'var(--hf-text-tertiary)' }}>No preview available</div>
      ...
      {proof.comments?.length > 0 && (
        <div style={{ background: 'var(--hf-surface)', border: '1px solid var(--hf-border)', borderRadius: 14, marginBottom: 20, overflow: 'hidden', animation: 'fadeIn 0.4s ease' }}>
              ...
              {proof.comments.map((c: any) => (
                <div key={c.id} id={`comment-${c.id}`}
                  style={{ display: 'flex', gap: 10, marginBottom: 12, borderRadius: 10, transition: 'background 0.3s',
                    background: highlightedCommentId === c.id ? 'var(--hf-warning-soft-strong)' : 'transparent', padding: highlightedCommentId === c.id ? 6 : 0 }}>
                  <div style={{ width: 30, height: 30, borderRadius: '50%', background: c.authorType === 'CLIENT' ? 'var(--hf-accent)' : 'var(--hf-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: 'var(--hf-text-on-solid)', fontWeight: 700, flexShrink: 0 }}>
                    {c.authorName.charAt(0).toUpperCase()}
                  </div>
                  <div style={{ flex: 1, background: 'var(--hf-surface-muted)', borderRadius: 10, padding: '10px 14px', border: '1px solid var(--hf-border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--hf-text)' }}>{c.authorName}</span>
                      <span style={{ fontSize: 11, color: 'var(--hf-text-faint)' }}>{c.authorType === 'CLIENT' ? 'You' : 'Designer'}</span>
                    </div>
                    {c.timecodeSeconds != null && (
                      <button
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 6, marginRight: 6, padding: '2px 8px', background: 'var(--hf-info-soft)', color: 'var(--hf-info-text)', border: 'none', borderRadius: 20, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                    ...
                    {c.anchorX != null && (
                      <button
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 6, padding: '2px 8px', background: 'var(--hf-accent-soft)', color: 'var(--hf-accent-text)', border: 'none', borderRadius: 20, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                        📍 Pinned comment
                      </button>
                    )}
                    <div style={{ fontSize: 13, color: 'var(--hf-text-secondary)', lineHeight: 1.6 }}>{c.comment}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div style={{ background: 'var(--hf-surface)', border: '1px solid var(--hf-border)', borderRadius: 14, padding: '18px 20px', marginBottom: 20, animation: 'fadeIn 0.45s ease' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {fileIsImage && (
            pendingPin ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--hf-accent-text)', fontWeight: 600 }}>
                📍 This comment will be pinned to that spot on the image
                <button onClick={() => setPendingPin(null)} style={{ background: 'none', border: 'none', color: 'var(--hf-text-faint)', fontSize: 12, cursor: 'pointer', textDecoration: 'underline', fontWeight: 400 }}>
                  Clear pin
                </button>
              </div>
            ) : (
          ...
          {fileIsVideo && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--hf-text-secondary)', cursor: 'pointer' }}>
              ...
              {tagTimecode && videoRef.current && (
                <span style={{ fontSize: 11, color: 'var(--hf-info-text)', fontWeight: 700 }}>
          ...
          {commentSent && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--hf-success-text-strong)', fontSize: 13, fontWeight: 600 }}>
          ...
          <button
            ...
            style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', background: !comment.trim() ? 'var(--hf-text-faint)' : 'var(--hf-primary)', color: 'var(--hf-text-on-solid)', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: !comment.trim() ? 'default' : 'pointer', transition: 'background 0.15s' }}>
      ...
      <div style={{ background: 'var(--hf-surface)', border: '1px solid var(--hf-border)', borderRadius: 14, padding: '22px 24px', animation: 'fadeIn 0.5s ease' }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--hf-text)', marginBottom: 6 }}>Ready to decide?</div>
        <div style={{ fontSize: 13, color: 'var(--hf-text-muted)', marginBottom: 20, lineHeight: 1.6 }}>
        ...
        {showReject && (
          <div style={{ background: 'var(--hf-warning-soft)', border: '1px solid var(--hf-orange-border)', borderRadius: 10, padding: '16px 18px', marginBottom: 18 }}>
            <label style={{ ...lbl, color: 'var(--hf-warning-text-deep)' }}>Describe the changes you need *</label>
            <textarea
              value={rejectReason}
              ...
              style={{ ...inp, resize: 'vertical' as const, border: '1.5px solid var(--hf-orange-border)', background: 'var(--hf-surface)' }}
            />
            <div style={{ fontSize: 12, color: 'var(--hf-warning-text-deep)', marginTop: 6 }}>
        ...
        {errorMsg && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'var(--hf-danger-soft)', border: '1px solid var(--hf-danger-border)', borderRadius: 8, fontSize: 13, color: 'var(--hf-danger-text)', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{ flex: 1, minWidth: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px 24px', background: !clientName.trim() ? 'var(--hf-text-faint)' : 'var(--hf-success-solid-strong)', color: 'var(--hf-text-on-solid)', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: !clientName.trim() ? 'default' : 'pointer', transition: 'all 0.15s' }}>
            {approve.isPending
          ...
          {!showReject ? (
            ...
            <div style={{ display: 'flex', gap: 8, flex: 1, minWidth: 160 }}>
              <button onClick={() => setShowReject(false)} style={{ flex: 1, padding: '14px', border: '1.5px solid var(--hf-border)', borderRadius: 10, background: 'var(--hf-surface)', fontSize: 14, cursor: 'pointer', fontWeight: 500, color: 'var(--hf-text-muted)' }}>
                Cancel
              </button>
              <button
                disabled={!rejectReason.trim() || !clientName.trim() || reject.isPending}
                onClick={() => reject.mutate()}
                style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '14px', background: !rejectReason.trim() || !clientName.trim() ? 'var(--hf-text-faint)' : 'var(--hf-danger)', color: 'var(--hf-text-on-solid)', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: (!rejectReason.trim() || !clientName.trim()) ? 'default' : 'pointer' }}>
        ...
        {/* Legal notice */}
        <div style={{ marginTop: 20, padding: '12px 14px', background: 'var(--hf-surface-muted)', border: '1px solid var(--hf-border)', borderRadius: 8, fontSize: 11, color: 'var(--hf-text-faint)', lineHeight: 1.7 }}>
...
function StatusCard({ icon, title, color, bg, children }: {
  ...
  return (
    <div style={{ background: 'var(--hf-surface)', border: '1px solid var(--hf-border)', borderRadius: 14, padding: '48px 36px', textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 14, color: 'var(--hf-text-muted)', lineHeight: 1.7, maxWidth: 480, margin: '0 auto' }}>{children}</div>
    </div>
  )
}
