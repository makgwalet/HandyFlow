...
const RISK_CFG: Record<string, { color: string; bg: string }> = {
  ...
  HIGH:   { color: "var(--hf-danger-text)", bg: "var(--hf-danger-soft)" },
}
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
        <div style={{ position: "relative" as const }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search clients..."
            style={{ paddingLeft: 28, padding: "7px 10px 7px 28px", border: "1px solid var(--hf-border)", borderRadius: 8, fontSize: 13, outline: "none", width: 240 }} />
        </div>
      ...
      {isLoading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map((c: any) => {
            ...
            return (
              <div key={c.id} style={{ border: "1px solid var(--hf-border)", borderLeft: `3px solid ${risk.color}`, borderRadius: 10, overflow: "hidden" }}>
                <div onClick={() => setExpanded(isOpen ? null : c.id)}
                  style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 20px", cursor: "pointer", background: isOpen ? "var(--hf-surface-muted)" : "var(--hf-surface)" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      <button onClick={e => { e.stopPropagation(); setWorkspaceFor(c) }}
                        style={{ fontWeight: 700, fontSize: 14, color: "var(--hf-primary-text)", background: "none", border: "none", padding: 0, cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 2 }}>
                        {c.tradingName}
                      </button>
                      <span style={{ background: "var(--hf-surface-muted)", color: "var(--hf-text-muted)", padding: "1px 7px", borderRadius: 20, fontSize: 11, border: "1px solid var(--hf-border)" }}>{c.entityType.replace("_"," ")}</span>
                      ...
                      {!c.ficaCompleted && <span style={{ background: "var(--hf-warning-soft)", color: "var(--hf-warning-text)", padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 700, border: "1px solid var(--hf-warning-border)" }}>FICA pending</span>}
                      {c.overdueDeadlines > 0 && <span style={{ background: "var(--hf-danger-soft)", color: "var(--hf-danger-text)", padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 700, border: "1px solid var(--hf-danger-border)" }}>{c.overdueDeadlines} overdue</span>}
                    </div>
                  ...
                  <div style={{ borderTop: "1px solid var(--hf-border)", padding: "16px 20px", background: "var(--hf-surface-muted)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 16 }}>
                      {[
                        ...
                        { l: "Overdue",        v: c.overdueDeadlines, color: c.overdueDeadlines > 0 ? "var(--hf-danger-text)" : undefined },
                      ].map((item: any) => (
                        <div key={item.l} style={{ background: "var(--hf-surface)", border: "1px solid var(--hf-border)", borderRadius: 7, padding: "8px 12px" }}>
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, color: item.color ?? (item.ok === false ? "var(--hf-danger-text)" : item.ok === true ? "var(--hf-success-text-strong)" : "var(--hf-text)") }}>
                            {item.v}
                          </div>
                        </div>
                      ))}
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {!c.ficaCompleted && (
                        <button onClick={() => markFica.mutate(c.id)}
                          style={{ padding: "6px 12px", background: "var(--hf-warning-soft)", color: "var(--hf-warning-text)", border: "1px solid var(--hf-warning-border)", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                          Mark FICA complete
                        </button>
                      )}
                      {!c.sarsAgentAppointed && (
                        <button onClick={() => markSars.mutate(c.id)}
                          style={{ padding: "6px 12px", background: "var(--hf-info-soft)", color: "var(--hf-info-text)", border: "1px solid var(--hf-info-border)", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                          Mark SARS agent appointed
                        </button>
                      )}
                      <button onClick={() => generateDeadlines.mutate(c.id)}
                        style={{ padding: "6px 12px", background: "var(--hf-success-soft)", color: "var(--hf-success-text-strong)", border: "1px solid var(--hf-success-border)", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                      ...
                      <label style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", background: "var(--hf-surface)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12, color: "var(--hf-text-secondary)", cursor: "pointer" }}>
                    ...
                    {docsOpenFor === c.id && (
                      <div style={{ marginTop: 14, padding: "12px 14px", background: "var(--hf-surface)", border: "1px solid var(--hf-border)", borderRadius: 9 }}>
                        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 10 }}>
                          <select value={docType} onChange={e => setDocType(e.target.value)}
                            style={{ padding: "6px 10px", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12, background: "var(--hf-surface)" }}>
                            {DOC_TYPES.map(t => <option key={t} value={t}>{t.replace(/_/g, " ")}</option>)}
                          </select>
                          <input type="date" value={docExpiry} onChange={e => setDocExpiry(e.target.value)}
                            placeholder="Expiry (optional)"
                            style={{ padding: "6px 10px", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }} />
                          <label style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", background: "var(--hf-info-soft)", color: "var(--hf-info-text)", border: "1px solid var(--hf-info-border)", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: uploadingDoc ? "default" : "pointer", opacity: uploadingDoc ? .6 : 1 }}>
                            <Paperclip size={12} />
                        ...
                        {docError && <div style={{ marginBottom: 10, padding: "8px 12px", background: "var(--hf-danger-soft)", border: "1px solid var(--hf-danger-border)", borderRadius: 7, fontSize: 12, color: "var(--hf-danger-text)" }}>{docError}</div>}
                        {ficaDocs.length === 0 ? (
                          <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No documents uploaded yet.</div>
                        ) : (
                          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            {ficaDocs.map((d: any) => (
                              <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7 }}>
                                <div>
                                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                                    <button onClick={() => downloadDoc(c.id, d)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "var(--hf-primary-text)", fontWeight: 600, padding: 0 }}>
                                      {d.fileName}
                                    </button>
                                    <span style={{ background: "var(--hf-surface-sunken)", color: "var(--hf-text-muted)", padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 700 }}>{d.docType.replace(/_/g, " ")}</span>
                                    {d.verified
                                      ? <span style={{ background: "var(--hf-success-soft-strong)", color: "var(--hf-success-text-strong)", padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", gap: 3 }}><CheckCircle size={9} /> Verified</span>
                                      : <span style={{ background: "var(--hf-warning-soft)", color: "var(--hf-warning-text)", padding: "1px 7px", borderRadius: 20, fontSize: 10, fontWeight: 700 }}>Unverified</span>}
                                  </div>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                  {!d.verified && (
                                    <button onClick={() => verifyDoc.mutate({ clientId: c.id, docId: d.id })}
                                      style={{ padding: "4px 10px", background: "var(--hf-success-soft)", color: "var(--hf-success-text-strong)", border: "1px solid var(--hf-success-border)", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                                      Verify
                                    </button>
                                  )}
      ...
      {showCreate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 680, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div>
                ...
                <select value={form.entityType} onChange={e => f("entityType", e.target.value)} style={{ ...inp, background: "var(--hf-surface)" }}>
                  {ENTITY_TYPES.map(t => <option key={t} value={t}>{t.replace("_"," ")}</option>)}
                </select>
              </div>
              <div>
                <label style={lbl}>Year-end month *</label>
                <select value={form.yearEndMonth} onChange={e => f("yearEndMonth", parseInt(e.target.value))} style={{ ...inp, background: "var(--hf-surface)" }}>
              ...
              <div>
                ...
                <select value={form.vatCategory} onChange={e => f("vatCategory", e.target.value)} style={{ ...inp, background: "var(--hf-surface)" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginTop: 4 }}>
              ...
              <label htmlFor="send-welcome-email" style={{ fontSize: 13, color: "var(--hf-text-secondary)", cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              ...
              <button disabled={!form.tradingName || createClient.isPending}
                ...
                style={{ padding: "9px 22px", background: !form.tradingName ? "var(--hf-text-faint)" : "var(--hf-primary)", color: "var(--hf-text-on-solid)", border: "none", borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
      ...
      {workspaceFor && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, width: 760, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ position: "sticky", top: 0, background: "var(--hf-surface)", borderBottom: "1px solid var(--hf-border)", padding: "20px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 1 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{workspaceFor.tradingName}</h3>
                <p style={{ margin: "3px 0 0", fontSize: 12, color: "var(--hf-text-muted)" }}>{workspaceFor.entityType?.replace("_", " ")} · {workspaceFor.riskRating} risk</p>
              </div>
              <button onClick={() => setWorkspaceFor(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--hf-text-faint)" }}><X size={20} /></button>
            </div>

            <div style={{ padding: "20px 28px" }}>
              {detailLoading ? (
                <div style={{ textAlign: "center", padding: 40, color: "var(--hf-text-faint)" }}>Loading client workspace...</div>
              ) : detailIsError ? (
                <div style={{ textAlign: "center", padding: 40, color: "var(--hf-danger-text)" }}>Couldn't load this client's workspace.</div>
              ) : clientDetail && (
                <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text-secondary)" }}>Deadlines</div>
                      {onNavigate && <button onClick={() => onNavigate("deadlines")} style={{ fontSize: 12, color: "var(--hf-primary-text)", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>View all →</button>}
                    </div>
                    {(clientDetail.recentDeadlines ?? []).length === 0 ? (
                      <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No deadlines recorded.</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {clientDetail.recentDeadlines.map((d: any) => (
                          <div key={d.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 12px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }}>
                            <span>{d.deadlineType} · {fmtD(d.adjustedDueDate)}</span>
                            <span style={{ fontWeight: 600, color: d.status === "OVERDUE" ? "var(--hf-danger-text)" : d.status === "FILED" ? "var(--hf-success-text-strong)" : "var(--hf-warning-text)" }}>{d.status}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text-secondary)" }}>Fee Notes</div>
                      {onNavigate && <button onClick={() => onNavigate("billing", workspaceFor.id)} style={{ fontSize: 12, color: "var(--hf-primary-text)", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>View all →</button>}
                    </div>
                    {(clientDetail.recentFeeNotes ?? []).length === 0 ? (
                      <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No fee notes yet.</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {clientDetail.recentFeeNotes.map((f: any) => (
                          <div key={f.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 12px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }}>
                            <span>{f.invoiceNumber} · {fmtD(f.invoiceDate)}</span>
                            <span style={{ display: "flex", gap: 10 }}>
                              <span style={{ fontWeight: 600 }}>{fmtR(f.total)}</span>
                              <span style={{ color: f.status === "PAID" ? "var(--hf-success-text-strong)" : f.status === "OVERDUE" ? "var(--hf-danger-text)" : "var(--hf-text-muted)" }}>{f.status}</span>
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text-secondary)" }}>Journals</div>
                      {onNavigate && <button onClick={() => onNavigate("journals")} style={{ fontSize: 12, color: "var(--hf-primary-text)", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>View all →</button>}
                    </div>
                    {(clientDetail.recentJournals ?? []).length === 0 ? (
                      <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No journals yet.</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {clientDetail.recentJournals.map((j: any) => (
                          <div key={j.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 12px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }}>
                            <span style={{ fontFamily: "monospace" }}>{j.reference} · {fmtD(j.journalDate)}</span>
                            <span style={{ color: j.status === "POSTED" ? "var(--hf-success-text-strong)" : "var(--hf-text-muted)" }}>{j.status}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text-secondary)" }}>Time</div>
                      {onNavigate && <button onClick={() => onNavigate("time", workspaceFor.id)} style={{ fontSize: 12, color: "var(--hf-primary-text)", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>View all →</button>}
                    </div>
                    {(clientDetail.recentTimeEntries ?? []).length === 0 ? (
                      <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No time logged yet.</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {clientDetail.recentTimeEntries.map((t: any) => (
                          <div key={t.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 12px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }}>
                            <span>{t.activityType?.replace("_", " ")} · {fmtD(t.entryDate)}</span>
                            <span style={{ display: "flex", gap: 10 }}>
                              <span style={{ fontWeight: 600 }}>{fmtR(t.lineTotal)}</span>
                              <span style={{ color: t.status === "BILLED" ? "var(--hf-success-text-strong)" : "var(--hf-text-muted)" }}>{t.status}</span>
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text-secondary)" }}>Portal Access</div>
                      <button onClick={() => { setShowInvite(true); setInviteError("") }}
                        style={{ fontSize: 12, color: "var(--hf-primary-text)", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>
                        + Invite to portal
                      </button>
                    </div>
                    {grantsLoading ? (
                      ...
                      <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No portal invites sent yet.</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {portalGrants.map((g: any) => {
                          const cfg: Record<string, { color: string; bg: string }> = {
                            ...
                            REVOKED: { color: "var(--hf-text-muted)", bg: "var(--hf-surface-sunken)" },
                          }
                          const c = cfg[g.status] ?? cfg.REVOKED
                          return (
                            <div key={g.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 12px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }}>
                              ...
                              <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                ...
                                {g.status !== "REVOKED" && (
                                  <button onClick={() => revokeMut.mutate(g.id)} disabled={revokeMut.isPending}
                                    style={{ background: "none", border: "none", cursor: "pointer", color: "var(--hf-danger-text)", fontSize: 11, fontWeight: 600 }}>
                                    Revoke
                                  </button>
                                )}
                              </span>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text-secondary)" }}>Document Requests</div>
                    ...
                    {docRequestsLoading ? (
                      ...
                      <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>No document requests yet.</div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {docRequests.map((r: any) => {
                          const cfg: Record<string, { color: string; bg: string }> = {
                            PENDING:   { color: "var(--hf-warning-text)", bg: "var(--hf-warning-soft)" },
                            PARTIAL:   { color: "var(--hf-info-text)", bg: "var(--hf-info-soft)" },
                            COMPLETE:  { color: "var(--hf-success-text-strong)", bg: "var(--hf-success-soft-strong)" },
                            CANCELLED: { color: "var(--hf-text-muted)", bg: "var(--hf-surface-sunken)" },
                          }
                          const c = cfg[r.status] ?? cfg.PENDING
                          return (
                            <div key={r.id} style={{ padding: "8px 12px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12 }}>
                              ...
                              {r.items?.length > 0 && (
                                <ul style={{ margin: 0, paddingLeft: 16, color: "var(--hf-text-muted)" }}>
      ...
      {showInvite && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Client contact email *</label>
            ...
            {inviteError && <div style={{ marginTop: 12, padding: "8px 12px", background: "var(--hf-danger-soft)", border: "1px solid var(--hf-danger-border)", borderRadius: 8, fontSize: 13, color: "var(--hf-danger-text)" }}>{inviteError}</div>}
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              <button onClick={() => setShowInvite(false)} style={{ padding: "9px 18px", border: "1px solid var(--hf-border)", borderRadius: 9, background: "var(--hf-surface)", fontSize: 14, cursor: "pointer" }}>Cancel</button>
              <button disabled={!inviteEmail || inviteMut.isPending}
                onClick={() => inviteMut.mutate()}
                style={{ padding: "9px 22px", background: !inviteEmail ? "var(--hf-text-faint)" : "var(--hf-primary)", color: "var(--hf-text-on-solid)", border: "none", borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
      ...
      {showNewDocRequest && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 460, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Items</label>
              {docRequestForm.items.map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                  <input value={item} onChange={e => setDocRequestForm(p => ({ ...p, items: p.items.map((it, idx) => idx === i ? e.target.value : it) }))}
                    ...
                    style={{ flex: 1, padding: "8px 10px", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 13 }} />
                  {docRequestForm.items.length > 1 && (
                    <button onClick={() => setDocRequestForm(p => ({ ...p, items: p.items.filter((_, idx) => idx !== i) }))}
                      style={{ background: "none", border: "none", cursor: "pointer", color: "var(--hf-danger-text)", padding: 4 }}>✕</button>
                  )}
                </div>
              ))}
              <button onClick={() => setDocRequestForm(p => ({ ...p, items: [...p.items, ""] }))}
                style={{ padding: "5px 10px", background: "var(--hf-surface-muted)", color: "var(--hf-text-secondary)", border: "1px solid var(--hf-border)", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                + Add item
              </button>
            </div>
            <div style={{ marginBottom: 6 }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Due date (optional)</label>
              <input type="date" value={docRequestForm.dueDate} onChange={e => setDocRequestForm(p => ({ ...p, dueDate: e.target.value }))}
                style={{ width: "100%", padding: "9px 12px", border: "1.5px solid var(--hf-border)", borderRadius: 8, fontSize: 14, boxSizing: "border-box" as const }} />
            </div>
            {docRequestError && <div style={{ marginTop: 12, padding: "8px 12px", background: "var(--hf-danger-soft)", border: "1px solid var(--hf-danger-border)", borderRadius: 8, fontSize: 13, color: "var(--hf-danger-text)" }}>{docRequestError}</div>}
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              <button onClick={() => setShowNewDocRequest(false)} style={{ padding: "9px 18px", border: "1px solid var(--hf-border)", borderRadius: 9, background: "var(--hf-surface)", fontSize: 14, cursor: "pointer" }}>Cancel</button>
