  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--hf-surface-sunken)", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: 860, margin: "0 auto", padding: "28px 24px" }}>
        <h1 style={{ fontSize: 20, fontWeight: 800, color: "var(--hf-text)", margin: "0 0 4px" }}>Your training account</h1>
        <p style={{ fontSize: 13, color: "var(--hf-text-faint)", margin: "0 0 24px" }}>Delegates, enrollments, certificates and invoicing for your organization.</p>

        <div style={{ display: "flex", gap: 4, borderBottom: "1px solid var(--hf-border)", marginBottom: 20 }}>
          {SUB_TABS.map(t => {
            const Icon = t.icon
        ...
        {sub === "delegates" && (
          delegatesLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          delegates.length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No delegates on file.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {delegates.map((d, i) => (
                <div key={d.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <div>
                    <p style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600, margin: 0 }}>{d.fullName}</p>
                    <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: 0 }}>{d.delegateNumber}{d.jobTitle ? ` · ${d.jobTitle}` : ""}</p>
                  </div>
                  {d.status === "INACTIVE" && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: "var(--hf-surface-sunken)", color: "var(--hf-text-muted)", alignSelf: "center" }}>INACTIVE</span>}
                </div>
              ))}
            </div>
          )
        ...
        {sub === "enrollments" && (
          enrollLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          (enrollData?.content ?? []).length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No enrollments yet.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {(enrollData?.content ?? []).map((e, i) => (
                <div key={e.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <span style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600 }}>{e.delegateNameSnapshot}</span>
                  <span style={{ fontSize: 11.5, color: "var(--hf-text-muted)" }}>{e.status.replace(/_/g, " ")}{e.score != null ? ` · Score ${e.score}` : ""}</span>
                </div>
              ))}
            </div>
          )
        ...
        {sub === "certificates" && (
          certLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          (certData?.content ?? []).length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No certificates issued yet.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {(certData?.content ?? []).map((c, i) => (
                <div key={c.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <div>
                    <p style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600, margin: 0 }}>{c.delegateNameSnapshot}</p>
                    <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: 0 }}>{c.courseTitleSnapshot} · {c.certificateNumber}</p>
                  </div>
                  <span style={{ fontSize: 11.5, color: c.status === "VALID" ? "var(--hf-success-text)" : "var(--hf-text-faint)", alignSelf: "center" }}>{c.status}</span>
                </div>
              ))}
            </div>
          )
        ...
        {sub === "invoices" && (
          invoicesLoading ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p> :
          invoices.length === 0 ? <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No invoices yet.</p> : (
            <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
              {invoices.map((inv, i) => (
                <div key={inv.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <div>
                    <p style={{ fontSize: 12.5, color: "var(--hf-text)", fontWeight: 600, margin: 0 }}>{inv.invoiceNumber}</p>
                    <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: 0 }}>{inv.periodStart} → {inv.periodEnd} · Due {inv.dueDate}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontSize: 12.5, fontWeight: 700, color: "var(--hf-text)", margin: 0 }}>{fmtMoney(inv.total)}</p>
                    <p style={{ fontSize: 11, color: inv.balance > 0 ? "var(--hf-warning-text)" : "var(--hf-success-text)", margin: 0 }}>{inv.balance > 0 ? `${fmtMoney(inv.balance)} due` : "Paid"}</p>
                  </div>
                </div>
              ))}
            </div>
