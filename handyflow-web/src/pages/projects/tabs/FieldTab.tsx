...
const SEVERITY:{[k:string]:{bg:string;color:string}}={LOW:{bg:'var(--hf-surface-sunken)',color:'var(--hf-text-tertiary)'},MEDIUM:{bg:'var(--hf-warning-soft-strong)',color:'var(--hf-warning-text-deep)'},HIGH:{bg:'var(--hf-danger-soft)',color:'var(--hf-danger-text)'},CRITICAL:{bg:'var(--hf-danger-soft)',color:'var(--hf-danger-text-strong)'}}
const SNAG_STATUS:{[k:string]:{bg:string;color:string}}={OPEN:{bg:'var(--hf-danger-soft)',color:'var(--hf-danger-text)'},IN_PROGRESS:{bg:'var(--hf-warning-soft-strong)',color:'var(--hf-warning-text-deep)'},RESOLVED:{bg:'var(--hf-success-soft-strong)',color:'var(--hf-success-text-strong)'},REJECTED:{bg:'var(--hf-surface-sunken)',color:'var(--hf-text-faint)'}}
      ...
      <div style={{display:'flex',gap:8,marginBottom:20}}>
        {([['diaries','📒 Site Diaries'],['snags','🔧 Snag List']] as const).map(([k,l])=>(
          <button key={k} onClick={()=>setPanel(k)}
            style={{padding:'8px 16px',borderRadius:8,border:panel===k?'1.5px solid var(--hf-primary)':'1px solid var(--hf-border)',background:panel===k?'var(--hf-info-soft)':'var(--hf-surface)',color:panel===k?'var(--hf-primary-text)':'var(--hf-text-muted)',fontSize:13,fontWeight:panel===k?700:500,cursor:'pointer'}}>
            {l}
          </button>
        ))}
      </div>
  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <div style={{fontSize:13,color:'var(--hf-text-muted)'}}>{diaries.length} site diaries submitted</div>
      ...
      {isLoading?<div style={{padding:40,textAlign:'center',color:'var(--hf-text-faint)'}}>Loading…</div>
        ...
        <div style={{display:'flex',flexDirection:'column' as const,gap:10}}>
          {diaries.map(d=>(
            <div key={d.id} style={{background:'var(--hf-surface)',border:'1px solid var(--hf-border)',borderRadius:10,padding:'14px 16px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                <div style={{fontSize:14,fontWeight:700,color:'var(--hf-text)'}}>{new Date(d.diaryDate).toLocaleDateString('en-ZA',{weekday:'long',day:'numeric',month:'long'})}</div>
                <div style={{display:'flex',gap:10,fontSize:12,color:'var(--hf-text-muted)'}}>
              ...
              {d.submittedByName&&<div style={{fontSize:11,color:'var(--hf-text-faint)',marginTop:8}}>Submitted by {d.submittedByName}</div>}
            </div>
          ))}
        </div>
      )}
      ...
      {showAdd&&(
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}}>
          <div style={{background:'var(--hf-surface)',borderRadius:14,padding:28,width:600,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Date</label><input type="date" value={form.diaryDate} onChange={e=>sf('diaryDate',e.target.value)} style={inp}/></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Weather</label>
              ...
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Temp °C</label><input type="number" value={form.tempCelsius} onChange={e=>sf('tempCelsius',e.target.value)} placeholder="22" style={inp}/></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Workers Present *</label><input type="number" value={form.workersPresent} onChange={e=>sf('workersPresent',e.target.value)} style={inp}/></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Workers Planned</label><input type="number" value={form.workersPlanned} onChange={e=>sf('workersPlanned',e.target.value)} style={inp}/></div>
              ...
              <div style={{gridColumn:'span 3'}}><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Progress Notes</label><textarea value={form.progressNotes} onChange={e=>sf('progressNotes',e.target.value)} style={{...inp,minHeight:50,resize:'vertical' as const}} placeholder="Progress vs plan…"/></div>
              <div style={{gridColumn:'span 3'}}><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Issues / Delays</label><textarea value={form.issues} onChange={e=>sf('issues',e.target.value)} style={{...inp,minHeight:50,resize:'vertical' as const}} placeholder="Any issues, delays or constraints…"/></div>
              <div style={{gridColumn:'span 3'}}><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Toolbox Talk Topic</label><input value={form.toolboxTopic} onChange={e=>sf('toolboxTopic',e.target.value)} placeholder="OHSA safety topic discussed" style={inp}/></div>
      ...
      {isLoading?<div style={{padding:40,textAlign:'center',color:'var(--hf-text-faint)'}}>Loading…</div>
        ...
        <div style={{display:'flex',flexDirection:'column' as const,gap:8}}>
          {snags.map(s=>{
            ...
            return (
              <div key={s.id} style={{background:'var(--hf-surface)',border:'1px solid var(--hf-border)',borderRadius:10,padding:'12px 14px'}}>
                <div style={{display:'flex',alignItems:'flex-start',gap:10}}>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                      <span style={{fontSize:11,color:'var(--hf-text-faint)',fontWeight:600}}>{s.snagNumber}</span>
                    ...
                    <div style={{fontSize:13,fontWeight:600,color:'var(--hf-text)',marginBottom:2}}>{s.title}</div>
                    <div style={{fontSize:12,color:'var(--hf-text-muted)'}}>{s.location&&`📍 ${s.location} · `}{s.assignedToName&&`👤 ${s.assignedToName}`}</div>
                  ...
                  <div style={{display:'flex',gap:6,marginTop:8}}>
                    {s.photoUrls.slice(0,4).map((url,i)=>(
                      <a key={i} href={url} target="_blank" rel="noreferrer">
                        <img src={url} alt="" style={{width:48,height:48,objectFit:'cover' as const,borderRadius:6,border:'1px solid var(--hf-border)'}} onError={e=>(e.currentTarget.style.display='none')}/>
                      </a>
                    ))}
                  </div>
                )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}}>
          <div style={{background:'var(--hf-surface)',borderRadius:14,padding:28,width:500,boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              <div style={{gridColumn:'span 2'}}><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Title *</label><input value={form.title} onChange={e=>sf('title',e.target.value)} placeholder="Cracked concrete slab" style={inp} autoFocus/></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Severity</label><select value={form.severity} onChange={e=>sf('severity',e.target.value)} style={inp}>{['LOW','MEDIUM','HIGH','CRITICAL'].map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Location</label><input value={form.location} onChange={e=>sf('location',e.target.value)} placeholder="Block A, Level 2" style={inp}/></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Assigned To</label><input value={form.assignedToName} onChange={e=>sf('assignedToName',e.target.value)} placeholder="Contractor name" style={inp}/></div>
              <div><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Due Date</label><input type="date" value={form.dueDate} onChange={e=>sf('dueDate',e.target.value)} style={inp}/></div>
              <div style={{gridColumn:'span 2'}}><label style={{display:'block',fontSize:12,fontWeight:600,color:'var(--hf-text-secondary)',marginBottom:5}}>Description</label><textarea value={form.description} onChange={e=>sf('description',e.target.value)} style={{...inp,minHeight:60,resize:'vertical' as const}}/></div>
            </div>
            {err&&<div style={{marginTop:10,padding:'8px 12px',background:'var(--hf-danger-soft)',border:'1px solid var(--hf-danger-border)',borderRadius:8,color:'var(--hf-danger-text)',fontSize:13}}>{err}</div>}
            <div style={{display:'flex',justifyContent:'flex-end',gap:10,marginTop:20}}>
              <button onClick={()=>setShowAdd(false)} style={{padding:'9px 16px',border:'1px solid var(--hf-border)',borderRadius:9,background:'var(--hf-surface)',fontSize:13,cursor:'pointer'}}>Cancel</button>
              <button onClick={()=>{if(!form.title.trim()){setErr('Title required');return}addMut.mutate({title:form.title.trim(),description:form.description||null,location:form.location||null,severity:form.severity,assignedTo:null,assignedToName:form.assignedToName||null,dueDate:form.dueDate||null})}} disabled={addMut.isPending} style={{padding:'9px 16px',background:'var(--hf-primary)',color:'var(--hf-text-on-solid)',border:'none',borderRadius:9,fontSize:13,fontWeight:600,cursor:'pointer',opacity:addMut.isPending?.6:1}}>{addMut.isPending?'Saving…':'Log Snag'}</button>
            </div>
          </div>
        </div>
      )}
...
function DiaryField({label,value,warn}:{label:string;value:string;warn?:boolean}){
  return <div style={{background:warn?'var(--hf-danger-soft)':'var(--hf-surface-muted)',borderRadius:8,padding:'8px 10px'}}><div style={{fontSize:10,fontWeight:600,color:warn?'var(--hf-danger-text)':'var(--hf-text-faint)',textTransform:'uppercase' as const,letterSpacing:'0.04em',marginBottom:3}}>{label}</div><div style={{fontSize:12,color:warn?'var(--hf-danger-text)':'var(--hf-text-secondary)'}}>{value}</div></div>
}
...
function Empty({icon:Icon,text,sub}:{icon:React.ElementType;text:string;sub:string}){return<div style={{textAlign:'center' as const,padding:'50px 20px',color:'var(--hf-text-faint)'}}><Icon size={36} style={{marginBottom:10,opacity:.3}}/><div style={{fontWeight:600,color:'var(--hf-text-tertiary)',marginBottom:4}}>{text}</div><div style={{fontSize:13}}>{sub}</div></div>}
