...
// Detail view embedded inside the Projects card — no separate route needed
import { useState } from "react"
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query"
import { apiClient } from "../../api/client"
import { ChevronLeft, Play, Pause, CheckCircle, ExternalLink, AlertTriangle } from "lucide-react"
...
export function ProjectDetailTab({ projectId, onBack }: { projectId:string; onBack:()=>void }) {
  ...
  return (
    <div>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:16 }}>
          ...
          <div style={{ display:"flex", gap:8, flexShrink:0 }}>
            {project.status === "PLANNING" && (
              <Btn onClick={() => actionMut.mutate("activate")} color="#166534" bg="#DCFCE7" border="#86EFAC" icon={Play}>Activate</Btn>
            )}
            {project.status === "ACTIVE" && (
              <Btn onClick={() => actionMut.mutate("hold")} color="#92400E" bg="#FEF3C7" border="#FCD34D" icon={Pause}>Hold</Btn>
            )}
            {project.status === "ON_HOLD" && (
              <Btn onClick={() => actionMut.mutate("activate")} color="#166534" bg="#DCFCE7" border="#86EFAC" icon={Play}>Resume</Btn>
            )}
      ...
      {/* Sub-tab bar — same pattern as the outer tab bar but smaller */}
      <div style={{ display:"flex", gap:2, borderBottom:"1px solid #E2E8F0", marginBottom:24, overflowX:"auto" }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ padding:"8px 14px", background:"none", border:"none", cursor:"pointer", fontSize:13, whiteSpace:"nowrap",
            ...
            {t.key==="risks" && project.openRiskCount > 0 && (
              <span style={{ marginLeft:5, background:"#FEF2F2", color:"#DC2626", fontSize:10, fontWeight:700, padding:"1px 5px", borderRadius:10 }}>{project.openRiskCount}</span>
            )}
          </button>
        ))}
      </div>
