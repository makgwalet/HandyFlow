...
const STATUS_CFG: Record<string, { color: string; bg: string; border: string; label: string }> = {
  RECEIVED:    { color: "var(--hf-info-text)", bg: "var(--hf-info-soft)", border: "var(--hf-info-border)", label: "Received"    },
  IN_PROGRESS: { color: "var(--hf-warning-text)", bg: "var(--hf-warning-soft)", border: "var(--hf-warning-border)", label: "In Progress"  },
  COMPLETED:   { color: "var(--hf-success-text-strong)", bg: "var(--hf-success-soft-strong)", border: "var(--hf-success-border)", label: "Completed"    },
  REJECTED:    { color: "var(--hf-danger-text)", bg: "var(--hf-danger-soft)", border: "var(--hf-danger-border)", label: "Rejected"     },
...
function EvidenceSection({ requestId, canManage }: { requestId: string; canManage: boolean }) {
  ...
  return (
    <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--hf-border)" }}>
      <div style={{ fontSize: 10, color: "var(--hf-text-faint)", fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 10 }}>Evidence — ID scans, request letters, correspondence</div>
      {evidence.length === 0 ? (
...
export default function DsarTab() {
  ...
  const stats = [
    { label: "Total",     value: requests.length,                                          color: "var(--hf-indigo-text)" },
    { label: "Open",      value: requests.filter(r => OPEN_STATUSES.has(r.status)).length,  color: "var(--hf-info-text)" },
    { label: "Overdue",   value: requests.filter(r => r.overdue).length,                    color: "var(--hf-danger-text)" },
    { label: "Completed", value: requests.filter(r => r.status === "COMPLETED").length,     color: "var(--hf-success-text-strong)" },
  ]
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          <button onClick={() => setOpenOnly(o => !o)}
            style={{ padding: "5px 12px", borderRadius: 20, fontSize: 12, cursor: "pointer", border: "none", fontWeight: openOnly ? 600 : 400,
              background: openOnly ? "var(--hf-indigo)" : "var(--hf-surface-sunken)", color: openOnly ? "var(--hf-text-on-solid)" : "var(--hf-text-muted)" }}>
            Open only
          </button>
      ...
      {isLoading ? (
        <div style={{ textAlign: "center", padding: 40, color: "var(--hf-text-faint)" }}>Loading requests...</div>
      ) : requests.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--hf-text-faint)" }}>
          <FileSearch size={40} style={{ marginBottom: 12, opacity: 0.4 }} />
          <div style={{ fontWeight: 600, color: "var(--hf-text-tertiary)" }}>No DSAR requests found</div>
        </div>
      ) : (
              ...
              <div key={r.id} style={{ border: `1px solid ${r.overdue ? "#FECACA" : "#E2E8F0"}`, borderRadius: 12, overflow: "hidden" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 700, fontSize: 15, color: "var(--hf-text)" }}>{r.requestNumber}</span>
                        <span style={{ fontSize: 14, color: "var(--hf-text-muted)" }}>{r.requesterName}</span>
                        <span style={{ fontSize: 11, color: "var(--hf-text-muted)", background: "var(--hf-surface-sunken)", padding: "1px 8px", borderRadius: 20 }}>{r.requestType}</span>
                        {r.overdue && <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 10, fontWeight: 700, background: "var(--hf-danger-soft)", color: "var(--hf-danger-text)", padding: "1px 7px", borderRadius: 20 }}><AlertTriangle size={10} />OVERDUE</span>}
                      </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                    {!isTerminal && (
                      <div style={{ textAlign: "right" as const }}>
                        <div style={{ fontWeight: 700, fontSize: 12, color: days < 0 ? "var(--hf-danger-text)" : days <= 5 ? "var(--hf-warning-text)" : "var(--hf-text)" }}>
                    ...
                    {canManage && (
                      <div style={{ display: "flex", gap: 5 }}>
                        {!isTerminal && <button onClick={() => { setShowAssign(r); setAssignUserId(r.assignedToUserId ?? ""); setAssignUserName(r.assignedToUserName ?? "") }} title="Assign" style={{ background: "var(--hf-violet-soft)", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "var(--hf-violet-text)" }}><UserPlus size={13} /></button>}
                        {!isTerminal && <button onClick={() => { setShowResolve({ request: r, action: "complete" }); setResolutionNotes("") }} title="Mark completed" style={{ background: "var(--hf-success-soft-strong)", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "var(--hf-success-text-strong)" }}><CheckCircle2 size={13} /></button>}
                        {!isTerminal && <button onClick={() => { setShowResolve({ request: r, action: "reject" }); setResolutionNotes("") }} title="Reject" style={{ background: "var(--hf-danger-soft)", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "var(--hf-danger-text)" }}><XCircle size={13} /></button>}
                        {!isTerminal && <button onClick={() => { setShowResolve({ request: r, action: "withdraw" }); setResolutionNotes("") }} title="Mark withdrawn" style={{ background: "var(--hf-surface-sunken)", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "var(--hf-text-muted)" }}><Undo2 size={13} /></button>}
                        {canAdmin && (
                          <button onClick={() => { if (confirm(`Delete request "${r.requestNumber}"?`)) deleteRequest.mutate(r.id) }} title="Delete" style={{ background: "var(--hf-danger-soft)", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "var(--hf-danger-text)" }}><Trash2 size={13} /></button>
                        )}
                      </div>
                    )}
                ...
                {isOpen && (
                  <div style={{ borderTop: "1px solid var(--hf-border-subtle)", padding: "16px 20px", background: "var(--hf-surface-muted)" }}>
                    ...
                    {r.resolutionNotes && <div style={{ padding: "8px 12px", background: "var(--hf-success-soft)", border: "1px solid var(--hf-success-border)", borderRadius: 8, fontSize: 13, color: "var(--hf-success-text-strong)" }}>Resolution: {r.resolutionNotes}</div>}
      ...
      {showAdd && (
        <Overlay onClose={() => { setShowAdd(false); setApiError("") }}>
          ...
          <div style={{ padding: "10px 12px", background: "var(--hf-indigo-soft)", border: "1px solid var(--hf-indigo-border)", borderRadius: 8, fontSize: 12, color: "var(--hf-indigo-text)", marginBottom: 16 }}>
            Due date is set automatically to 30 days after the received date.
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div>
              <label style={lbl}>Request type *</label>
              <select value={form.requestType} onChange={e => setForm(f => ({ ...f, requestType: e.target.value }))} style={{ ...inp("requestType"), background: "var(--hf-surface)" }}>
      ...
      {showResolve && (
        <Overlay onClose={() => { setShowResolve(null); setApiError("") }}>
          ...
          <label style={lbl}>Resolution notes <span style={{ fontWeight: 400, color: "var(--hf-text-faint)" }}>(optional)</span></label>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            <button onClick={() => { setShowResolve(null); setApiError("") }} style={{ padding: "9px 18px", border: "1px solid var(--hf-border)", borderRadius: 9, background: "var(--hf-surface)", fontSize: 14, cursor: "pointer", color: "var(--hf-text-secondary)" }}>Cancel</button>
            <button onClick={() => resolve.mutate({ id: showResolve.request.id, action: showResolve.action, notes: resolutionNotes })} disabled={resolve.isPending}
              style={{ padding: "9px 22px", background: resolve.isPending ? "var(--hf-text-faint)" : actionColor[showResolve.action], color: "var(--hf-text-on-solid)", border: "none", borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: resolve.isPending ? "not-allowed" : "pointer" }}>
...
function Overlay({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(2px)" }}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 600, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>{children}</div>
    </div>
  )
}
