...
export default function ConsultationsTab() {
  ...
  return (
    <div>
      ...
      {!selectedPatientId ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--hf-text-faint)", border: "1px dashed var(--hf-border)", borderRadius: 12 }}>
          <FileText size={40} style={{ marginBottom: 12, opacity: 0.4 }} />
          <div style={{ fontWeight: 600, color: "var(--hf-text-tertiary)" }}>Select a patient to view their consultation history</div>
        ...
        <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--hf-text-faint)", border: "1px dashed var(--hf-border)", borderRadius: 12 }}>
          <div style={{ fontWeight: 600, color: "var(--hf-text-tertiary)" }}>No consultations recorded for this patient</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {(consultations as Consultation[]).map(c => {
            ...
            return (
              <div key={c.id} style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
                {/* Header */}
                <div onClick={() => setExpanded(isOpen ? null : c.id)}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <button onClick={e => { e.stopPropagation(); setShowRx(c.id); setApiError("") }}
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", background: "var(--hf-success-soft)", color: "var(--hf-success-text-strong)", border: "1px solid var(--hf-success-border)", borderRadius: 6, fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                      <Pill size={11} /> Rx
                    </button>
                    ...
                    <button onClick={e => { e.stopPropagation(); setShowReferral(c.id); setReferralError(""); setReferralForm({ specialistName: "", specialty: "", reason: "", urgency: "ROUTINE", additionalNotes: "" }) }}
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", background: "var(--hf-danger-soft)", color: "var(--hf-danger-text-strong)", border: "1px solid var(--hf-danger-border)", borderRadius: 6, fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                    ...
                    <button onClick={async e => {
                      ...
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", background: "var(--hf-surface-muted)", color: "var(--hf-primary-text)", border: "1px solid var(--hf-border)", borderRadius: 6, fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                  ...
                  <div style={{ borderTop: "1px solid var(--hf-border)", padding: "18px 20px", background: "var(--hf-surface-muted)" }}>
                    ...
                    {(c.weightKg || c.bloodPressure || c.pulseBpm || c.temperatureC || c.oxygenSatPct) && (
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "var(--hf-text-faint)", letterSpacing: "0.06em", marginBottom: 8 }}>VITALS</div>
                        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                          {[
                            ...
                            <div key={l} style={{ background: "var(--hf-surface)", border: "1px solid var(--hf-border)", borderRadius: 8, padding: "8px 14px", minWidth: 80, textAlign: "center" as const }}>
                              <div style={{ fontSize: 10, color: "var(--hf-text-faint)", marginBottom: 2 }}>{l}</div>
                    ...
                    {c.weightKg && c.heightCm && (
                      <div style={{ marginBottom: 12, fontSize: 12, color: "var(--hf-text-muted)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      {[
                        ...
                        <div key={l}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: "var(--hf-text-faint)", textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 3 }}>{l}</div>
                          <div style={{ fontSize: 13, color: "var(--hf-text)", lineHeight: 1.5 }}>{v}</div>
                        </div>
                      ))}
                    </div>
                    {c.icd10Codes?.length > 0 && (
                      <div style={{ marginTop: 12 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "var(--hf-text-faint)", textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 5 }}>ICD-10 Codes</div>
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          {c.icd10Codes.map(code => (
                            <span key={code} style={{ background: "var(--hf-info-soft)", color: "var(--hf-info-text)", padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 600, border: "1px solid var(--hf-info-border)" }}>{code}</span>
                          ))}
                        </div>
                      </div>
                    )}
            ...
            {(prescriptions as Prescription[]).length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
                {(prescriptions as Prescription[]).map(rx => (
                  <div key={rx.id} style={{ border: "1px solid var(--hf-border)", borderRadius: 10, padding: "14px 16px", background: rx.dispensed ? "var(--hf-success-soft)" : "var(--hf-surface)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14, color: "var(--hf-text)", marginBottom: 4 }}>{rx.medicationName}</div>
                        <div style={{ fontSize: 12, color: "var(--hf-text-muted)" }}>
                          {[rx.dosage, rx.frequency, rx.duration].filter(Boolean).join(" · ")}
                          {rx.quantity ? ` · Qty: ${rx.quantity}` : ""}
                          {rx.repeats > 0 ? ` · Repeats: ${rx.repeats}` : ""}
                        </div>
                        {rx.instructions && <div style={{ fontSize: 12, color: "var(--hf-text-tertiary)", marginTop: 6, fontStyle: "italic" }}>{rx.instructions}</div>}
                      </div>
                      {rx.dispensed && (
                        <span style={{ background: "var(--hf-success-soft-strong)", color: "var(--hf-success-text-strong)", padding: "2px 8px", borderRadius: 20, fontSize: 11, fontWeight: 700, flexShrink: 0 }}>DISPENSED</span>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {showReferral && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 560, maxHeight: "85vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            {referralError && <div style={{ marginTop: 14, padding: "8px 12px", background: "var(--hf-danger-soft)", border: "1px solid var(--hf-danger-border)", borderRadius: 7, fontSize: 13, color: "var(--hf-danger-text)" }}>{referralError}</div>}
          ...
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 720, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <FormSection title="Patient & Practitioner">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                <div>
                  ...
                  <select value={form.practitionerId} onChange={e => f("practitionerId", e.target.value)} style={{ ...inp("practitionerId"), background: "var(--hf-surface)" }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  {fieldErrors.chiefComplaint && <div style={{ fontSize: 12, color: "var(--hf-danger-text)", marginTop: 4 }}>{fieldErrors.chiefComplaint}</div>}
                </div>
              </div>
            </FormSection>
            ...
            <FormSection title="Vitals">
              ...
              {form.weightKg && form.heightCm && (
                <div style={{ marginTop: 10, fontSize: 12, color: "var(--hf-text-muted)", background: "var(--hf-surface-muted)", padding: "6px 12px", borderRadius: 7, display: "inline-block" }}>
            ...
            <FormSection title="Clinical Notes">
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div><label style={lbl}>ICD-10 Codes <span style={{ fontWeight: 400, color: "var(--hf-text-faint)" }}>(comma separated)</span></label><input value={form.icd10Codes} onChange={e => f("icd10Codes", e.target.value)} placeholder="J06.9, Z00.0" style={sinp} /></div>
...
function FormSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 22 }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: "var(--hf-text-faint)", letterSpacing: "0.07em", textTransform: "uppercase" as const, marginBottom: 12, paddingBottom: 8, borderBottom: "1px solid var(--hf-border-subtle)" }}>{title}</div>
      {children}
    </div>
  )
}
