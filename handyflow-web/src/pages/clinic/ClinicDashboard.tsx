...
export default function ClinicDashboard({ onNavigate }: { onNavigate: (tab: any) => void }) {
  ...
  const upcoming = appts.filter((a: any) => ["SCHEDULED","CONFIRMED"].includes(a.status) && a.scheduledAt >= new Date().toISOString())
    .sort((a: any, b: any) => a.scheduledAt.localeCompare(b.scheduledAt))

  const kpis = [
    { label: "Today's appointments", value: todayA.length, color: "#1B3A6B", bg: "#EFF6FF", icon: Calendar, tab: "appointments" },
    { label: "Awaiting today",        value: todayA.filter((a: any) => ["SCHEDULED","CONFIRMED"].includes(a.status)).length, color: "#D97706", bg: "#FFFBEB", icon: Clock, tab: "appointments" },
    { label: "Completed today",       value: todayA.filter((a: any) => a.status === "COMPLETED").length, color: "#166534", bg: "#DCFCE7", icon: CheckCircle, tab: "appointments" },
    { label: "Total patients",        value: (patientsData as any)?.totalElements ?? "—", color: "#7C3AED", bg: "#F5F3FF", icon: Users, tab: "patients" },
  ]
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 28 }}>
        {kpis.map(k => (
          <div key={k.label} onClick={() => onNavigate(k.tab)}
            style={{ background: k.bg, borderRadius: 12, padding: "18px 20px", cursor: "pointer", border: `1px solid ${k.bg}` }}
        ...
        <div>
          ...
          {todayA.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", border: "1px dashed #E2E8F0", borderRadius: 12, color: "#94A3B8" }}>
              ...
              <button onClick={() => onNavigate("appointments")}
                style={{ marginTop: 12, padding: "7px 16px", background: "#0D9488", color: "#fff", border: "none", borderRadius: 7, fontSize: 13, cursor: "pointer", fontWeight: 600 }}>
                Book appointment
              </button>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {todayA.sort((a: any,b: any) => a.scheduledAt.localeCompare(b.scheduledAt)).map((a: any) => {
                ...
                return (
                  <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", border: "1px solid #E2E8F0", borderRadius: 10, background: "#fff" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                        ...
                        <span style={{ fontSize: 10, fontWeight: 600, background: s.bg, color: s.color, padding: "1px 7px", borderRadius: 20 }}>{s.label}</span>
                      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {upcoming[0] && (
            <div style={{ background: "#0D9488", borderRadius: 12, padding: 20, color: "#fff" }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.6)", marginBottom: 8, textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>Next up</div>
              <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 3 }}>{upcoming[0].patientName}</div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.75)", marginBottom: 12 }}>{upcoming[0].appointmentType?.replace("_"," ")}</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{fmtTime(upcoming[0].scheduledAt)}</div>
              {upcoming[0].practitionerName && (
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginTop: 6 }}>Dr. {upcoming[0].practitionerName}</div>
              )}
            </div>
          )}
          ...
          <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 12, padding: 16 }}>
            ...
            {(practitioners as any[]).slice(0, 5).map((p: any) => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: "1px solid #F1F5F9" }}>
                <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E0F2FE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#0369A1", flexShrink: 0 }}>
                  {p.firstName?.[0]}{p.lastName?.[0]}
                </div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#0F172A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{p.fullName}</div>
                  <div style={{ fontSize: 11, color: "#94A3B8" }}>{p.specialty}</div>
                </div>
              </div>
            ))}
            {practitioners.length === 0 && <div style={{ fontSize: 13, color: "#94A3B8" }}>No practitioners added</div>}
          </div>
