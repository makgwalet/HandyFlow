      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: `${space(10)} ${space(5)} ${space(12)}` }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center" as const, padding: `${space(14)} ${space(6)}`, background: color.surface, border: `1px dashed ${color.border}`, borderRadius: radius.lg }}>
            ...
            <div style={{ fontSize: 13.5, color: color.muted, maxWidth: 320, margin: "0 auto" }}>Once your payroll bureau grants you access, it will appear here.</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: space(3) }}>
            {clients.map(c => (
              <button key={c.clientId} onClick={() => navigate(`/payroll-bureau/portal/clients/${c.clientId}`)}
                ...
                <div style={{ fontSize: 12.5, color: color.faint, fontWeight: 500, marginTop: space(3) }}>View invoices and SARS deadlines →</div>
              </button>
            ))}
          </div>
        )}
