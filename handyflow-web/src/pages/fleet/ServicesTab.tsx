  ...
  return (
    <div>
      ...
      {selectedVehicleObj && (services as any[]).length > 0 && (
        <div style={{ display: "flex", gap: 16, marginBottom: 20, padding: "14px 18px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 10, flexWrap: "wrap" }}>
          {[
            ...
            <div key={item.l} style={{ borderRight: "1px solid var(--hf-border)", paddingRight: 16, marginRight: 4 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--hf-text)" }}>{item.v}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {!selectedVehicle ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--hf-text-faint)" }}>
          ...
          <div style={{ fontWeight: 600, color: "var(--hf-text-tertiary)" }}>Select a vehicle to view service history</div>
        </div>
      ) : isLoading ? (
        <div style={{ textAlign: "center", padding: 40, color: "var(--hf-text-faint)" }}>Loading...</div>
      ) : (services as any[]).length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--hf-text-faint)", border: "1px dashed var(--hf-border)", borderRadius: 12 }}>No service records for this vehicle.</div>
      ) : (
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                      ...
                      <span style={{ fontWeight: 600, fontSize: 14, color: "var(--hf-text)" }}>{s.description}</span>
                    </div>
                    ...
                    {s.nextServiceKm && (
                      <div style={{ fontSize: 12, color: "var(--hf-accent-text)", marginTop: 2 }}>
                        Next service: {Number(s.nextServiceKm).toLocaleString()} km
                      </div>
                    )}
                  </div>
                </div>
                <div style={{ fontWeight: 700, fontSize: 15, color: "var(--hf-text)", flexShrink: 0 }}>{fmtR(s.cost)}</div>
              </div>
            )
          })}
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 560, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
              <div>
                <h3 style={{ margin: "0 0 3px", fontSize: 17, fontWeight: 700, color: "var(--hf-text)" }}>Record Service</h3>
                <div style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>{selectedVehicleObj.registration} — {selectedVehicleObj.make} {selectedVehicleObj.model} · {Number(selectedVehicleObj.currentOdometer).toLocaleString()} km</div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--hf-text-faint)", marginTop: 3 }}>Auto-calculated: +{(selectedVehicleObj.serviceIntervalKm || 10000).toLocaleString()} km</div>
              </div>
              <div>
