...
const STATUS_COLORS: Record<string, { bg: string; fg: string }> = {
  EXPECTED: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-tertiary)" }, PARTIALLY_RECEIVED: { bg: "var(--hf-warning-soft-strong)", fg: "var(--hf-warning-text-deep)" },
  RECEIVED: { bg: "var(--hf-success-soft-strong)", fg: "var(--hf-success-text-strong)" }, CANCELLED: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-faint)" },
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 560, maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: "var(--hf-text)", margin: 0 }}>New inbound shipment</p>
        ...
        <p style={{ fontSize: 12.5, fontWeight: 700, color: "var(--hf-text)", margin: "16px 0 8px" }}>Expected lines</p>
        {lines.map((line, i) => (
        ...
        {create.isError && <p style={{ color: "var(--hf-danger-text)", fontSize: 12, marginTop: 12 }}>{(create.error as any)?.response?.data?.message ?? "Could not create this shipment"}</p>}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: "var(--hf-text-faint)", margin: 0 }}>{shipments.length} shipment{shipments.length === 1 ? "" : "s"}</p>
      ...
      {isLoading ? (
        ...
        <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No inbound shipments yet.</p>
      ) : (
        <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
          {shipments.map((s, i) => {
            const colors = STATUS_COLORS[s.status] ?? { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-muted)" }
            return (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <p style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text)", margin: 0 }}>{s.referenceNumber ?? `Shipment ${s.id.slice(0, 8)}`}</p>
                    ...
                    <p style={{ fontSize: 11.5, color: "var(--hf-text-faint)", margin: 0 }}>{s.expectedDate ? `Expected ${s.expectedDate}` : "No expected date set"}</p>
                  </div>
                </div>
