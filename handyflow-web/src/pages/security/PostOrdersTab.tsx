...
const modalBox: React.CSSProperties = { background: "#fff", borderRadius: 14, padding: 26, width: 540, maxHeight: "85vh", overflowY: "auto" as const }

function StatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { bg: string; color: string }> = {
    DRAFT: { bg: "#F1F5F9", color: "#64748B" },
    ACTIVE: { bg: "#DCFCE7", color: "#166534" },
    SUPERSEDED: { bg: "#FFFBEB", color: "#B45309" },
  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      <div style={{ marginBottom: 18 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#0F172A" }}>Post Orders</h1>
        <div style={{ fontSize: 13, color: "#64748B", marginTop: 2 }}>Site and post instructions — versioned, acknowledgment-tracked. The guard-facing "My Post" view lives in the Shield app.</div>
      </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 20, borderBottom: "1px solid #E2E8F0" }}>
            {([["site-order", "Site-Level Order", MapPin], ["posts", "Posts", Users2], ["contacts", "Contacts", Phone]] as const).map(([id, label, Icon]) => (
              <button key={id} onClick={() => setSubTab(id)}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 16px", background: "none", border: "none", borderBottom: subTab === id ? "2px solid #1B3A6B" : "2px solid transparent", color: subTab === id ? "#1B3A6B" : "#64748B", fontWeight: subTab === id ? 700 : 500, fontSize: 13, cursor: "pointer" }}>
                <Icon size={15} /> {label}
              </button>
            ))}
          </div>
  ...
  return (
    <div>
            ...
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontWeight: 700, fontSize: 15, color: "#0F172A" }}>Version {current.version}</span>
              ...
              {current.contacts.length > 0 && (
                <div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" as const, marginBottom: 6 }}>Contacts</div>
                  <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6 }}>
                    {current.contacts.map(c => (
                      <span key={c.id} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 20, background: "#F1F5F9", color: "#374151" }}>
              ...
              {showAcks && (
                <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid #F1F5F9" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" as const, marginBottom: 8 }}>
                    Acknowledged v{current.version} — {acksQuery.data?.length ?? 0} guards
                  </div>
                  {!acksQuery.data?.length ? (
                    <p style={{ fontSize: 12, color: "#94A3B8" }}>No guard has acknowledged this version yet.</p>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                      {acksQuery.data.map(a => (
                        <div key={a.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "6px 10px", background: "#F8FAFC", borderRadius: 6 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        {draft ? (
          <>
            <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#B45309" }}>
            ...
            <button onClick={() => publish.mutate(draft.id)} disabled={publish.isPending}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "#1B3A6B", color: "#fff", border: "none", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
        ...
        {history.length > 0 && (
          <button onClick={() => setShowHistory(v => !v)}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", border: "1px solid #E2E8F0", background: "#fff", color: "#64748B", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
      ...
      {showHistory && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {history.map(o => (
            <div key={o.id} style={{ display: "flex", justifyContent: "space-between", padding: "9px 12px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, fontSize: 12 }}>
              <span>Version {o.version}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ color: "#94A3B8" }}>{o.publishedAt ? new Date(o.publishedAt).toLocaleDateString() : "unpublished"}</span>
      ...
      {showNewDraft && (
        <div style={modalOverlay}>
          <div style={modalBox}>
            <h3 style={{ margin: "0 0 6px", fontSize: 16, fontWeight: 700, color: "#0F172A" }}>{postId ? "New Post Order Version" : "New Site Order Version"}</h3>
            <p style={{ fontSize: 11, color: "#94A3B8", marginBottom: 16 }}>Prefilled from the current version — edit and save as a draft, then publish separately when ready.</p>
                ...
                {contacts.map(c => {
                  ...
                  return (
                    <button key={c.id} type="button"
                      ...
                      style={{ fontSize: 11, padding: "5px 11px", borderRadius: 20, border: selected ? "1px solid #1B3A6B" : "1px solid #E2E8F0", background: selected ? "#EFF6FF" : "#fff", color: selected ? "#1B3A6B" : "#64748B", cursor: "pointer", fontWeight: selected ? 700 : 500 }}>
                      {c.name}
                    </button>
                  )
                })}
                {contacts.length === 0 && <span style={{ fontSize: 12, color: "#94A3B8" }}>No contacts yet for this site — add some in the Contacts tab.</span>}
              </div>
            </div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" as const, marginBottom: 3 }}>{label}</div>
      <div style={{ fontSize: 13, color: "#374151", whiteSpace: "pre-wrap" as const }}>{value}</div>
    </div>
  )
}
      ...
      {showNew && (
        <div style={modalOverlay}>
          <div style={{ ...modalBox, width: 400 }}>
            <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 700, color: "#0F172A" }}>New Post</h3>
            {error && <p style={{ color: "#DC2626", fontSize: 12, marginBottom: 12 }}>{error}</p>}
...
function ContactsPanel({ siteId, qc }: { siteId: string; qc: ReturnType<typeof useQueryClient> }) {
  ...
  return (
    <div>
      ...
      {error && <div style={{ padding: "10px 14px", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 8, color: "#DC2626", fontSize: 13, marginBottom: 14 }}>{error}</div>}
      <p style={{ fontSize: 12, color: "#94A3B8", marginBottom: 14 }}>Reusable across every post order at this site — update a contact once, every order referencing it stays current.</p>
      ...
      {isLoading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {contacts.map(c => (
            <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "#fff", border: "1px solid #E2E8F0", borderRadius: 10 }}>
              <div>
                <span style={{ fontWeight: 700, fontSize: 14, color: "#0F172A" }}>{c.name}</span>
                <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: "#F1F5F9", color: "#64748B" }}>{c.role.replace(/_/g, " ")}</span>
                {!c.siteId && <span style={{ marginLeft: 6, fontSize: 10, color: "#94A3B8" }}>(tenant-wide)</span>}
              </div>
              <span style={{ fontSize: 12, color: "#64748B" }}>{c.phone}{c.email ? ` · ${c.email}` : ""}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {showNew && (
        <div style={modalOverlay}>
          <div style={{ ...modalBox, width: 400 }}>
            <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 700, color: "#0F172A" }}>New Contact</h3>
            {error && <p style={{ color: "#DC2626", fontSize: 12, marginBottom: 12 }}>{error}</p>}
            <div style={{ marginBottom: 14 }}>
              <label style={lbl}>Name *</label>
