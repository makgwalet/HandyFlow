...
import { Plus, X, AlertTriangle, CheckCircle, Clock, AlertCircle } from "lucide-react"
...
export default function IncidentsTab() {
  ...
  const { data: incidents = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (r.data?.data ?? r.data).content ?? []
    },
  })

  const { data: sites = [] }  = useQuery({ queryKey: ["sites-list"],  queryFn: async () => { const r = await apiClient.get("/api/v1/security/sites?size=100"); return (r.data?.data ?? r.data).content ?? [] } })
  const { data: guards = [] } = useQuery({ queryKey: ["guards-list"], queryFn: async () => { const r = await apiClient.get("/api/v1/security/guards?size=100"); return (r.data?.data ?? r.data).content ?? [] } })

  const createIncident = useMutation({
    mutationFn: (body: any) => apiClient.post("/api/v1/security/incidents", body),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["incidents"] }); setShowCreate(false); setFormError("") },
  ...
  // Stats
  const total    = (incidents as any[]).length
  const open     = (incidents as any[]).filter(i => i.status === "OPEN").length
  const ack      = (incidents as any[]).filter(i => i.status === "ACKNOWLEDGED").length
              ...
              <div key={inc.id} style={{ border: `1px solid ${sev.border}`, borderLeft: `4px solid ${sev.color}`, borderRadius: 10, padding: "16px 20px", background: "#fff" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    {inc.status === "OPEN" && (
                      <button onClick={() => acknowledge.mutate(inc.id)}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  ...
                  <select value={form.guardId} onChange={e => setForm(f => ({ ...f, guardId: e.target.value }))} style={inp}>
                    ...
                    {(guards as any[]).map((g: any) => <option key={g.id} value={g.id}>{g.fullName}</option>)}
                  </select>
                </div>
              </div>
              <div>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              ...
              <button
                onClick={() => createIncident.mutate({
                  siteId: form.siteId || null, guardId: form.guardId || null,
