// src/pages/training/TrainingCertificatesTab.tsx
//
// All certificates across every session — confirmed via
// TrainingCertificateController: GET /api/v1/training/certificates,
// POST /certificates/{id}/revoke (ADMIN-only, CancelRequest reason),
// GET /certificates/{id}/pdf. Issuance itself happens from
// TrainingSessionDetail (it's a per-enrollment action), not here.
...
interface CertificateResponse {
  id: string; enrollmentId: string; employeeId: string; employeeNameSnapshot: string; courseTitleSnapshot: string
...
export default function TrainingCertificatesTab() {
  ...
  const { data, isLoading } = useQuery<CertificatePage>({
    queryKey: ["training-certificates", statusFilter],
    queryFn: async () => (await apiClient.get("/api/v1/training/certificates", { params: { status: statusFilter || undefined, size: 100 } })).data,
  })
  const certificates = data?.content ?? []

  // ADMIN-only server-side (TRAINING_ADMIN) — same note as issuance.
  const revoke = useMutation({
    mutationFn: async (id: string) => {
      const reason = prompt("Reason for revoking this certificate:") ?? undefined
      return apiClient.post(`/api/v1/training/certificates/${id}/revoke`, reason ? { reason } : undefined)
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["training-certificates"] }),
  })

  const downloadPdf = async (cert: CertificateResponse) => {
    const res = await apiClient.get(`/api/v1/training/certificates/${cert.id}/pdf`, { responseType: "blob" })
