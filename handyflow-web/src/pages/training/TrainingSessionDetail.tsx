      ...
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 460 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
          ...
          <button onClick={() => save.mutate()} disabled={save.isPending}
            style={{ flex: 2, padding: "10px", borderRadius: 8, border: "none", background: TRAINING_ACCENT, color: "var(--hf-text-on-solid)", fontSize: 13, fontWeight: 700, cursor: "pointer", opacity: save.isPending ? 0.6 : 1 }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 440 }}>
        <p style={{ fontSize: 15, fontWeight: 800, color: "var(--hf-text)", margin: "0 0 18px" }}>Enrol an employee</p>
        ...
        {enroll.isError && <p style={{ color: "var(--hf-danger-text)", fontSize: 12, marginTop: 12 }}>{(enroll.error as any)?.response?.data?.message ?? "Could not enrol this employee"}</p>}
        <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
          <button onClick={onClose} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "1px solid var(--hf-border)", background: "var(--hf-surface)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
          <button onClick={() => enroll.mutate()} disabled={!employee || enroll.isPending}
            style={{ flex: 2, padding: "10px", borderRadius: 8, border: "none", background: TRAINING_ACCENT, color: "var(--hf-text-on-solid)", fontSize: 13, fontWeight: 700, cursor: "pointer", opacity: (!employee || enroll.isPending) ? 0.6 : 1 }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 380 }}>
        ...
        <p style={{ fontSize: 12, color: "var(--hf-text-faint)", margin: "0 0 18px" }}>{enrollment.employeeNameSnapshot}</p>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
          <button onClick={() => setPassed(true)} style={{ flex: 1, padding: "9px", borderRadius: 8, border: passed ? `1.5px solid ${TRAINING_ACCENT}` : "1px solid var(--hf-border)", background: passed ? "var(--hf-success-soft)" : "var(--hf-surface)", color: passed ? TRAINING_ACCENT : "var(--hf-text-muted)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Passed</button>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "20px 0 10px" }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text)", margin: 0 }}>Enrolled employees ({enrollments.length})</p>
        {session.status !== "CANCELLED" && session.status !== "COMPLETED" && (
          <button onClick={() => setShowEnroll(true)} style={{ display: "flex", alignItems: "center", gap: 6, background: TRAINING_ACCENT, color: "var(--hf-text-on-solid)", border: "none", borderRadius: 8, padding: "8px 14px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
            ...
            <div key={e.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: "var(--hf-text)", margin: 0 }}>{e.employeeNameSnapshot}</p>
                ...
                {e.employeeNumberSnapshot && <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: "2px 0 0" }}>{e.employeeNumberSnapshot}</p>}
              </div>
