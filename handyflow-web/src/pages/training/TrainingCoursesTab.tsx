// src/pages/training/TrainingCoursesTab.tsx
//
// Course catalogue CRUD — confirmed via TrainingCourseController:
// GET/POST /api/v1/training/courses, PUT/{id}, POST/{id}/archive,
// POST/{id}/reactivate, DELETE/{id} (ADMIN-only server-side).
// UpsertCourseRequest is the shared create/update body.
...
export interface CourseResponse {
  id: string; courseCode: string; title: string; description: string | null; category: string | null
  deliveryMode: string; durationHours: number | null; defaultTrainerName: string | null; cost: number | null
  certificationOffered: boolean; certificateValidityMonths: number | null; status: "ACTIVE" | "ARCHIVED"; createdAt: string
}
...
const emptyForm = {
  title: "", description: "", category: "", deliveryMode: "IN_PERSON", durationHours: "",
  defaultTrainerName: "", cost: "", certificationOffered: false, certificateValidityMonths: "",
}
...
function CourseFormModal({ initial, onClose }: { initial?: CourseResponse; onClose: () => void }) {
  ...
  const [form, setForm] = useState(() => initial ? {
    ...
    deliveryMode: initial.deliveryMode, durationHours: initial.durationHours?.toString() ?? "",
    defaultTrainerName: initial.defaultTrainerName ?? "", cost: initial.cost?.toString() ?? "",
  ...
  const save = useMutation({
    mutationFn: async () => {
      const body = {
        ...
        deliveryMode: form.deliveryMode, durationHours: toNum(form.durationHours),
        defaultTrainerName: form.defaultTrainerName || null, cost: toNum(form.cost),
        certificationOffered: form.certificationOffered, certificateValidityMonths: toInt(form.certificateValidityMonths),
      }
      return initial
        ? apiClient.put(`/api/v1/training/courses/${initial.id}`, body)
        : apiClient.post("/api/v1/training/courses", body)
    },
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 520, maxHeight: "85vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          <div><label style={labelStyle}>Title *</label><input style={inputStyle} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="First Aid Level 1" /></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><label style={labelStyle}>Category</label><input style={inputStyle} value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} placeholder="Compliance, Technical…" /></div>
            <div>
              <label style={labelStyle}>Delivery mode</label>
              <select style={inputStyle} value={form.deliveryMode} onChange={e => setForm({ ...form, deliveryMode: e.target.value })}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><label style={labelStyle}>Duration (hours)</label><input type="number" step="0.5" style={inputStyle} value={form.durationHours} onChange={e => setForm({ ...form, durationHours: e.target.value })} /></div>
            <div><label style={labelStyle}>Default trainer</label><input style={inputStyle} value={form.defaultTrainerName} onChange={e => setForm({ ...form, defaultTrainerName: e.target.value })} /></div>
          </div>
          <div><label style={labelStyle}>Cost per employee (informational — not posted to the GL)</label><input type="number" step="0.01" style={inputStyle} value={form.cost} onChange={e => setForm({ ...form, cost: e.target.value })} /></div>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--hf-text-secondary)" }}>
        ...
        {save.isError && <p style={{ color: "var(--hf-danger-text)", fontSize: 12, marginTop: 12 }}>{(save.error as any)?.response?.data?.message ?? "Could not save this course"}</p>}

        <button onClick={() => save.mutate()} disabled={!form.title || save.isPending}
          style={{ marginTop: 18, width: "100%", padding: "11px", borderRadius: 8, border: "none", background: TRAINING_ACCENT, color: "var(--hf-text-on-solid)", fontSize: 13.5, fontWeight: 700, cursor: "pointer", opacity: (!form.title || save.isPending) ? 0.6 : 1 }}>
...
export default function TrainingCoursesTab() {
  ...
  const { data, isLoading } = useQuery<CoursePage>({
    queryKey: ["training-courses", statusFilter],
    queryFn: async () => (await apiClient.get("/api/v1/training/courses", { params: { status: statusFilter || undefined, size: 100 } })).data,
  })
  const courses = data?.content ?? []

  const archive = useMutation({
    mutationFn: async (id: string) => apiClient.post(`/api/v1/training/courses/${id}/archive`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["training-courses"] }),
  })
  const reactivate = useMutation({
    mutationFn: async (id: string) => apiClient.post(`/api/v1/training/courses/${id}/reactivate`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["training-courses"] }),
  })
  // ADMIN-only server-side (TRAINING_ADMIN) — real gate is the backend @PreAuthorize,
  // same note as every other provider/internal module built in this engagement.
  const remove = useMutation({
    mutationFn: async (id: string) => apiClient.delete(`/api/v1/training/courses/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["training-courses"] }),
  })
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <p style={{ fontSize: 13, color: "var(--hf-text-faint)", margin: 0 }}>{courses.length} course{courses.length === 1 ? "" : "s"}</p>
      ...
      {isLoading ? (
        ...
        <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No courses yet.</p>
      ) : (
            ...
            <div key={c.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: "var(--hf-success-soft)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <GraduationCap size={15} color={TRAINING_ACCENT} />
                </div>
                <div>
                  ...
                  <p style={{ fontSize: 11.5, color: "var(--hf-text-faint)", margin: 0 }}>
                    {c.category ?? "Uncategorised"} · {c.deliveryMode.replace("_", " ")} {c.durationHours ? `· ${c.durationHours}h` : ""} {c.cost != null ? `· ${fmtMoney(c.cost)}` : ""}
                  </p>
                </div>
              </div>
