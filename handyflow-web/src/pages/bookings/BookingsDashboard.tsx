...
export default function BookingsDashboard({ onNavigate }: { onNavigate: (tab: any) => void }) {
  const { data: todayData } = useQuery({
    ...
    queryFn: async () => {
        const res = await apiClient.get(`/api/v1/bookings?date=${today}&size=50`)
        const payload = res.data?.data ?? res.data        // unwrap ApiResponse wrapper
        return (payload?.content ?? payload) as any[]     // unwrap Page wrapper
        },
  })

  const { data: allData } = useQuery({
    queryKey: ["bookings-all-summary"],
    queryFn: async () => {
  ...
  const todayRevenue     = allBookings.filter(b => b.status === "COMPLETED" && b.bookingDate === today).reduce((s: number, b: any) => s + (b.price ?? 0), 0)
  const weekCompleted    = allBookings.filter(b => b.status === "COMPLETED").length
  const noShowRate       = allBookings.length > 0 ? ((allBookings.filter(b => b.status === "NO_SHOW").length / allBookings.length) * 100).toFixed(0) : "0"
        ...
        {[
          ...
          <div key={k.label}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#94A3B8", textTransform: "uppercase" as const }}>{k.label}</div>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "#0F172A" }}>
              Today's Schedule — {new Date().toLocaleDateString("en-ZA", { weekday: "long", day: "numeric", month: "long" })}
            </span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {upcoming && (
            <div style={{ background: "#1B3A6B", borderRadius: 12, padding: 20, color: "#fff" }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.6)", marginBottom: 8, textTransform: "uppercase" as const }}>Next up</div>
          ...
          <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 12, padding: 18 }}>
            ...
            {[
              { label: "Total completed",  value: weekCompleted,    color: "#166534" },
              { label: "No-show rate",     value: `${noShowRate}%`, color: parseFloat(noShowRate) > 10 ? "#DC2626" : "#166534" },
              { label: "Total bookings",   value: allBookings.length, color: "#1B3A6B" },
            ].map(s => (
