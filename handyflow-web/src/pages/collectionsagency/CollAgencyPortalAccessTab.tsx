...
const STATUS_COLORS: Record<string, { bg: string; fg: string }> = {
  PENDING: { bg: "var(--hf-warning-soft-strong)", fg: "var(--hf-warning-text-deep)" }, ACTIVE: { bg: "var(--hf-success-soft-strong)", fg: "var(--hf-success-text-strong)" }, REVOKED: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-muted)" },
}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }} onClick={onClose}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 26, width: 400 }} onClick={e => e.stopPropagation()}>
        ...
        {save.isError && <p style={{ color: "var(--hf-danger-text)", fontSize: 12, marginTop: 10 }}>{(save.error as any)?.response?.data?.message ?? "Could not send this invite"}</p>}
...
export default function CollAgencyPortalAccessTab({ clientId }: { clientId: string }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: "var(--hf-text-muted)", margin: 0 }}>{grants.length} portal invite{grants.length === 1 ? "" : "s"}</p>
        <button onClick={() => setShowInvite(true)} style={{ display: "flex", alignItems: "center", gap: 6, background: CA_ACCENT, color: "var(--hf-text-on-solid)", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      {isLoading ? (
        ...
        <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No one has been invited to this client's portal yet.</p>
      ) : (
              ...
              <div key={g.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11.5, color: "var(--hf-text-faint)", margin: "2px 0 0" }}>
                ...
                {g.status !== "REVOKED" && (
                  <button onClick={() => revoke.mutate(g.id)} style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "1px solid var(--hf-danger-border)", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, color: "var(--hf-danger-text)", cursor: "pointer" }}>
