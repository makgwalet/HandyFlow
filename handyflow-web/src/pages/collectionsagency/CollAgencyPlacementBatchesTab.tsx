...
const inputStyle: React.CSSProperties = { width: "100%", padding: "7px 10px", border: "1px solid var(--hf-border)", borderRadius: 6, fontSize: 12.5, boxSizing: "border-box", fontFamily: "inherit" }
const labelStyle: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: "var(--hf-text-secondary)", marginBottom: 3, display: "block" }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }} onClick={onClose}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 26, width: 720, maxHeight: "88vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ fontSize: 12, fontWeight: 700, color: "var(--hf-text)", margin: "0 0 8px" }}>Debtor accounts in this batch ({lines.length})</p>
        <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: "0 0 10px" }}>Leave "original creditor" blank to default to this client's own trading name — the usual case when the client IS the original creditor.</p>
        ...
        <button onClick={() => setLines([...lines, emptyLine()])} style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "1px dashed var(--hf-border-strong)", borderRadius: 8, padding: "8px 14px", fontSize: 12, fontWeight: 600, color: "var(--hf-text-muted)", cursor: "pointer", marginBottom: 16 }}>
        ...
        {save.isError && <p style={{ color: "var(--hf-danger-text)", fontSize: 12, marginTop: 10 }}>{(save.error as any)?.response?.data?.message ?? "Could not place this batch"}</p>}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: "var(--hf-text-muted)", margin: 0 }}>{batches.length} batch{batches.length === 1 ? "" : "es"} placed</p>
        <button onClick={() => setShowNew(true)} style={{ display: "flex", alignItems: "center", gap: 6, background: CA_ACCENT, color: "var(--hf-text-on-solid)", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      {isLoading ? (
        ...
        <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No batches placed yet.</p>
      ) : (
        <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
          {batches.map((b, i) => (
            <div key={b.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text)", margin: "0 0 2px" }}>
                  {b.batchReference || `Batch placed ${b.placedDate}`} — {b.totalAccounts} account{b.totalAccounts === 1 ? "" : "s"}
                </p>
                <p style={{ fontSize: 12, color: "var(--hf-text-faint)", margin: 0 }}>Placed {b.placedDate} · Total value {fmtMoney(b.totalPlacedValue)}</p>
              </div>
              {b.acknowledgedAt ? (
