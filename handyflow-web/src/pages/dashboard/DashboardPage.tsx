...
// ─────────────────────────────────────────────────────────────────────────────
// MODULE_REGISTRY
// Keys MUST match moduleKey values returned by GET /api/v1/billing/modules/mine
// Missing keys are silently filtered out — that's why tiles don't appear.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: hovered ? '0 8px 24px rgba(27,58,107,0.12)' : '0 1px 4px rgba(0,0,0,0.04)',
...
export function DashboardPage() {
  ...
  const { data: customersData } = useQuery({
    ...
    queryFn: async () => { const res = await apiClient.get('/api/v1/crm/customers?size=100'); return res.data.content as { id: string; createdAt: string }[] },
  })
  const { data: quotesData } = useQuery({
    queryKey: ['dashboard-quotes'],
    queryFn: async () => { const res = await apiClient.get('/api/v1/invoicing/quotes?size=100'); return res.data.content as { id: string; status: string; total: number }[] },
  })
  const { data: invoicesData } = useQuery({
    queryKey: ['dashboard-invoices'],
    queryFn: async () => { const res = await apiClient.get('/api/v1/invoicing/invoices?size=100'); return res.data.content as { id: string; status: string; total: number; createdAt: string }[] },
  })
  ...
  const revenueMTD     = invoices.filter(i => i.status === 'PAID' && new Date(i.createdAt) >= startOfMonth).reduce((s, i) => s + (i.total || 0), 0)
  ...
  const { data: tenantModules = [] } = useQuery<{ moduleKey: string; accessible: boolean }[]>({
    ...
    queryFn: async () => { const r = await apiClient.get('/api/v1/billing/modules/mine'); return r.data || [] },
    staleTime: 5 * 60 * 1000,
  })

  // CRM and Catalogue always show regardless of billing API response
  ...
  const activeApps = allActiveKeys
    .filter(key => MODULE_REGISTRY[key])   // silently drops keys not in registry
    ...
    <div style={{ minHeight: '100vh', background: '#F1F5F9', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '36px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14, marginBottom: 36 }}>
          {activeApps.map(app => <AppTileCard key={app.key} app={app} onClick={() => navigate(app.route)} />)}
        </div>
