...
const STATUS_COLORS: Record<string, { bg: string; fg: string }> = {
  PLACED: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-tertiary)" }, IN_PROGRESS: { bg: "var(--hf-info-soft-strong)", fg: "var(--hf-info-text)" },
  PAYMENT_PLAN_ACTIVE: { bg: "var(--hf-warning-soft-strong)", fg: "var(--hf-warning-text-deep)" }, DISPUTED: { bg: "var(--hf-danger-soft-strong)", fg: "var(--hf-danger-text-strong)" },
  RECOVERED: { bg: "var(--hf-success-soft-strong)", fg: "var(--hf-success-text-strong)" }, RETURNED_TO_CLIENT: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-muted)" },
  WRITTEN_OFF: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-muted)" }, CLOSED: { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-muted)" },
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--hf-surface-sunken)", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: 860, margin: "0 auto", padding: "28px 24px" }}>
        <button onClick={() => navigate("/collections-agency/portal")} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "var(--hf-text-muted)", fontSize: 13, marginBottom: 18, padding: 0 }}>
          <ArrowLeft size={15} /> All accounts
        </button>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 28 }}>
          <div style={{ background: "var(--hf-surface)", border: "1px solid var(--hf-primary-border)", borderRadius: 14, padding: 18 }}>
            <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: "0 0 6px", fontWeight: 700, textTransform: "uppercase" }}>Currently outstanding</p>
            <p style={{ fontSize: 22, fontWeight: 800, color: "var(--hf-text)", margin: 0 }}>{fmtMoney(totalOutstanding)}</p>
          </div>
          <div style={{ background: "var(--hf-surface)", border: "1px solid var(--hf-primary-border)", borderRadius: 14, padding: 18 }}>
            <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: "0 0 6px", fontWeight: 700, textTransform: "uppercase" }}>Recovered to date</p>
            <p style={{ fontSize: 22, fontWeight: 800, color: "var(--hf-success-text)", margin: 0 }}>{fmtMoney(totalRecovered)}</p>
          </div>
        </div>

        <p style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text)", margin: "0 0 10px" }}>Placed accounts ({accounts.length})</p>
        {accountsLoading ? (
          <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p>
        ) : accounts.length === 0 ? (
          <p style={{ color: "var(--hf-text-faint)", fontSize: 13, marginBottom: 24 }}>No accounts placed with this agency yet.</p>
        ) : (
          <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden", marginBottom: 28 }}>
            {accounts.map((a, i) => {
              const colors = STATUS_COLORS[a.status] ?? { bg: "var(--hf-surface-sunken)", fg: "var(--hf-text-muted)" }
              return (
                <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: "var(--hf-text)", margin: 0 }}>{a.debtorName}</p>
                    ...
                    <p style={{ fontSize: 11.5, color: "var(--hf-text-faint)", margin: 0 }}>{a.accountReference ? `${a.accountReference} · ` : ""}Placed {a.placedDate}</p>
                  </div>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text)", margin: 0 }}>{fmtMoney(a.currentBalance)}</p>
                </div>
              )
            })}
          </div>
        ...
        <p style={{ fontSize: 13, fontWeight: 700, color: "var(--hf-text)", margin: "0 0 10px" }}>Trust statement</p>
        {trustLoading ? (
          <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>Loading…</p>
        ) : trust.length === 0 ? (
          <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No trust movements yet.</p>
        ) : (
          <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
            {trust.map((t, i) => (
              <div key={t.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
                ...
                <p style={{ fontSize: 13, fontWeight: 700, color: t.transactionType === "RECEIPT" ? "var(--hf-success-text)" : "var(--hf-warning-text)", margin: 0 }}>
