  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--hf-text)', margin: '0 0 4px' }}>Billing & Apps</h1>
        <p style={{ fontSize: 13, color: 'var(--hf-text-faint)', margin: 0 }}>Manage your active apps and subscription</p>
      </div>
        ...
        <div style={{ background: 'white', border: '1px solid var(--hf-border)', borderRadius: 14, padding: '20px 24px', marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 24 }}>
            {[
              ...
              <div key={label}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--hf-text-faint)', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: 3 }}>{label}</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--hf-text)' }}>{value}</div>
              </div>
            ))}
          </div>
          {subscription.status === 'PILOT' && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 38, fontWeight: 900, color: 'var(--hf-warning-text)', lineHeight: 1 }}>{subscription.pilotDaysRemaining}</div>
              <div style={{ fontSize: 11, color: 'var(--hf-text-faint)', fontWeight: 600 }}>DAYS LEFT</div>
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--hf-border)', marginBottom: 24 }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ padding: '10px 20px', background: 'none', border: 'none', borderBottom: tab === t.id ? '2px solid var(--hf-accent)' : '2px solid transparent', color: tab === t.id ? 'var(--hf-accent-text)' : 'var(--hf-text-muted)', fontWeight: tab === t.id ? 700 : 400, fontSize: 14, cursor: 'pointer', marginBottom: -1 }}>
            {t.label}
            {t.id === 'modules' && <span style={{ marginLeft: 6, background: 'var(--hf-surface-sunken)', color: 'var(--hf-text-muted)', padding: '1px 7px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{tenantModules.length + CORE_KEYS.length}</span>}
            {t.id === 'subscription' && availableToAdd.length > 0 && <span style={{ marginLeft: 6, background: 'var(--hf-info-soft)', color: 'var(--hf-info-text)', padding: '1px 7px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{availableToAdd.length} available</span>}
          </button>
        ))}
      </div>
      ...
      {tab === 'modules' && (
        <div>
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--hf-text-faint)', letterSpacing: '0.06em', marginBottom: 12 }}>CORE — ALWAYS INCLUDED FREE</div>
                  ...
                  <div key={key} style={{ background: 'white', border: '1px solid var(--hf-border)', borderRadius: 12, padding: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--hf-text-faint)' }}>{descs[key]}</div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--hf-success-text-strong)' }}>Included free</span>
                      <span style={{ background: 'var(--hf-success-soft-strong)', color: 'var(--hf-success-text-strong)', padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>FREE</span>
                    </div>
                  </div>
                )
              })}
          ...
          {/* Active paid apps */}
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--hf-text-faint)', letterSpacing: '0.06em', marginBottom: 12 }}>
              ...
              {tenantModules.map(m => {
                ...
                return (
                  <div key={m.moduleKey} style={{ background: 'white', border: `1px solid ${isExpiring ? '#FCD34D' : '#E2E8F0'}`, borderRadius: 12, padding: '16px', position: 'relative' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--hf-text-faint)' }}>{m.description}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--hf-text)' }}>
                          R {m.monthlyPrice}/mo
                        </div>
                        {isTrial && m.trialEndsAt && (
                          <div style={{ fontSize: 11, color: isExpiring ? 'var(--hf-warning-text)' : 'var(--hf-text-muted)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                            {isExpiring && <AlertTriangle size={11} />}
                            {daysLeft}d trial left · until {fmtDate(m.trialEndsAt)}
                          </div>
                        )}
                        {m.status === 'ACTIVE' && (
                          <div style={{ fontSize: 11, color: 'var(--hf-text-muted)', marginTop: 2 }}>Active · renews {fmtDate(m.activatedAt)}</div>
                        )}
                      </div>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <span style={{
                          background: isTrial ? 'var(--hf-warning-soft-strong)' : 'var(--hf-success-soft-strong)',
                          color:      isTrial ? 'var(--hf-warning-text-deep)' : 'var(--hf-success-text-strong)',
                        ...
                        <button
                          ...
                          title="Remove app"
                          style={{ background: 'var(--hf-danger-soft)', border: '1px solid var(--hf-danger-border)', borderRadius: 7, padding: '5px 8px', cursor: 'pointer', color: 'var(--hf-danger-text)', display: 'flex' }}>
                          <X size={13} />
                        </button>
                      </div>
                    </div>
        ...
        <div>
          {availableToAdd.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--hf-text-faint)' }}>
              <Check size={40} style={{ marginBottom: 12, color: 'var(--hf-accent-text)', opacity: 0.6 }} />
              <div style={{ fontWeight: 600, color: 'var(--hf-text-tertiary)', marginBottom: 4 }}>All apps are active</div>
                      ...
                      <div key={m.key} style={{ background: 'white', border: '1px solid var(--hf-border)', borderRadius: 12, padding: '16px' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 12 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 12, color: 'var(--hf-text-faint)', lineHeight: 1.4 }}>{m.description}</div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--hf-text)' }}>R {Number(m.monthlyPrice).toLocaleString('en-ZA')}/mo</div>
                            <div style={{ fontSize: 11, color: 'var(--hf-accent-text)', fontWeight: 500 }}>60-day free trial</div>
                          </div>
                          <button
      ...
      {confirmCancel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: 460, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--hf-text-muted)', marginBottom: 20 }}>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'white', borderRadius: 16, padding: 28, width: 440, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
                ...
                <button onClick={() => setConfirmActivate(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--hf-text-faint)', display: 'flex' }}><X size={20} /></button>
              </div>
              ...
              <div style={{ fontSize: 13, color: 'var(--hf-text-muted)', marginBottom: 20, lineHeight: 1.5 }}>
