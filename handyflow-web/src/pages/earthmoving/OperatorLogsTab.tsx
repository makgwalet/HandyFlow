    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <label style={{ fontSize: 14, fontWeight: 500, color: "var(--hf-text-secondary)" }}>Equipment:</label>
      ...
      {selectedAsset && logs.length > 0 && (
        <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
          {[
            { label: "Total Sessions", value: logs.length, color: "var(--hf-primary-text)" },
            { label: "Total Hours", value: `${totalHours.toFixed(1)} hrs`, color: "var(--hf-accent-text)" },
            { label: "Unique Operators", value: uniqueOps, color: "var(--hf-violet-text)" },
          ].map(s => (
      ...
      {!selectedAsset ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--hf-text-faint)" }}>
          ...
          <div style={{ fontWeight: 600, color: "var(--hf-text-tertiary)" }}>Select equipment to view operator logs</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {logs.map((log, i) => {
                ...
                return (
                  <tr key={log.id} style={{ borderBottom: i < logs.length - 1 ? "1px solid var(--hf-border-subtle)" : "none", background: isOpen ? "var(--hf-success-soft)" : "var(--hf-surface)" }}>
                    <td style={{ padding: "13px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 30, height: 30, borderRadius: "50%", background: "var(--hf-info-soft)", border: "2px solid var(--hf-info-border)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 11, color: "var(--hf-info-text)", flexShrink: 0 }}>
                          {(log.operatorName || "?")[0].toUpperCase()}
                        </div>
                        <span style={{ fontWeight: 600, color: "var(--hf-text)" }}>{log.operatorName || "—"}</span>
                        {isOpen && (
                          <span style={{ fontSize: 10, fontWeight: 700, background: "var(--hf-success-soft-strong)", color: "var(--hf-success-text-strong)", padding: "1px 7px", borderRadius: 20 }}>ACTIVE</span>
                        )}
                      </div>
                    </td>
                    <td style={{ padding: "13px 16px", color: "var(--hf-text-tertiary)" }}>{log.siteName || "—"}</td>
                    <td style={{ padding: "13px 16px", color: "var(--hf-text-tertiary)", fontSize: 12 }}>{fmtDate(log.startedAt)}</td>
                    <td style={{ padding: "13px 16px", color: "var(--hf-text-tertiary)", fontFamily: "monospace" }}>{fmtTime(log.startedAt)}</td>
                    <td style={{ padding: "13px 16px", color: "var(--hf-text-tertiary)", fontFamily: "monospace" }}>{log.endedAt ? fmtTime(log.endedAt) : "—"}</td>
                    <td style={{ padding: "13px 16px", fontWeight: 600, color: "var(--hf-accent-text)" }}>
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      {isOpen && (
                        <button
                          ...
                          style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", background: "var(--hf-success-solid-strong)", color: "var(--hf-text-on-solid)", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
      ...
      {showAdd && selectedAssetObj && (
        <Overlay onClose={() => setShowAdd(false)} width={480}>
          ...
          <div style={{ marginBottom: 16, padding: "10px 14px", background: "var(--hf-sky-soft)", border: "1px solid var(--hf-sky-border)", borderRadius: 8, fontSize: 13, color: "var(--hf-sky-text-strong)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--hf-text-faint)", marginTop: 3 }}>Pre-filled from current reading</div>
              </div>
            </div>
          </div>
