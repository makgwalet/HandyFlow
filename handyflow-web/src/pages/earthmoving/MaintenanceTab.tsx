...
export default function MaintenanceTab() {
  ...
  return (
    <div>
      ...
      {selectedAssetObj && (
        <div style={{ marginBottom: 18, padding: "14px 18px", background: "var(--hf-surface-muted)", border: "1px solid var(--hf-border)", borderRadius: 10, display: "flex", gap: 24, flexWrap: "wrap" }}>
          {[
            ...
            <div key={item.l}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: selectedAssetObj.dueForService && item.l === "Hours Since Service" ? "var(--hf-danger-text)" : "var(--hf-text)" }}>{item.v}</div>
            </div>
          ))}
          {selectedAssetObj.dueForService && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--hf-danger-soft)", color: "var(--hf-danger-text)", padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700 }}>
      ...
      {!selectedAsset ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--hf-text-faint)" }}>
          ...
          <div style={{ fontWeight: 600, color: "var(--hf-text-tertiary)" }}>Select equipment to view maintenance history</div>
        </div>
      ) : isLoading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {records.map(r => {
            ...
            return (
              <div key={r.id} style={{ background: "var(--hf-surface)", border: "1px solid var(--hf-border)", borderRadius: 12, padding: "14px 18px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ display: "flex", gap: 12 }}>
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        ...
                        <span style={{ fontWeight: 600, fontSize: 14, color: "var(--hf-text)" }}>{r.description}</span>
                      </div>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 700, color: "var(--hf-text)", flexShrink: 0 }}>{fmtCurrency2dp(r.cost)}</div>
                </div>
              </div>
            )
          })}
