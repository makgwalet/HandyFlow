...
export function BillsTab({ onRefreshSummary }: { onRefreshSummary: () => void }) {
  ...
  return (
    <div>
      ...
      {duplicateWarning && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12,
          ...
          <button onClick={() => setDuplicateWarning("")}
            style={{ background: "none", border: "none", cursor: "pointer", color: "var(--hf-warning-text)", display: "flex" }}>
            <X size={14} />
          </button>
        </div>
      )}
      ...
      {/* Bills table */}
      {isLoading ? (
        <div style={{ textAlign: "center", padding: 48, color: "var(--hf-text-faint)" }}>Loading bills...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px" }}>
          <FileText size={40} style={{ marginBottom: 12, color: "var(--hf-text-disabled)" }} />
          <div style={{ fontWeight: 700, color: "var(--hf-text-tertiary)", fontSize: 15, marginBottom: 6 }}>No bills found</div>
              ...
              {filtered.map((b, i) => {
                ...
                return (
                  <tr key={b.id} onClick={() => setSelected(b)}
                    ...
                    <td style={{ padding: "12px 16px", fontSize: 12, color: "var(--hf-text-muted)" }}>{fmtDate(b.billDate)}</td>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ fontSize: 12, color: b.overdue ? "var(--hf-danger-text)" : b.daysUntilDue <= 7 && !["PAID","CANCELLED"].includes(b.status) ? "var(--hf-warning-text)" : "var(--hf-text-muted)", fontWeight: b.overdue ? 700 : 400 }}>
                        {fmtDate(b.dueDate)}
                      </div>
                      {!["PAID","CANCELLED"].includes(b.status) && (
                        <div style={{ fontSize: 10, color: b.overdue ? "var(--hf-danger-text)" : "var(--hf-text-faint)" }}>
                          {b.overdue ? `${Math.abs(b.daysUntilDue)}d overdue` : `${b.daysUntilDue}d left`}
                        </div>
                      )}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ fontWeight: 800, color: "var(--hf-text)" }}>{fmtR(b.totalAmount)}</div>
                      {b.vatAmount > 0 && <div style={{ fontSize: 10, color: "var(--hf-text-faint)" }}>excl. VAT {fmtR(b.amount)}</div>}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      ...
                      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                        {["DRAFT","SECOND_APPROVAL"].includes(b.status) && (
                          <button onClick={e => { e.stopPropagation(); setSelected(b); setShowApprove(true) }}
                            style={{ padding: "4px 10px", background: "var(--hf-success-soft-strong)", color: "var(--hf-success-text-strong)", border: "1px solid var(--hf-success-border)", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 3 }}>
                        ...
                        {["APPROVED","OVERDUE"].includes(b.status) && !b.batchId && (
                          <button onClick={e => { e.stopPropagation(); setSelected(b); setShowPayModal(true) }}
                            style={{ padding: "4px 10px", background: "var(--hf-info-soft)", color: "var(--hf-info-text)", border: "1px solid var(--hf-info-border)", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 3 }}>
            ...
            {filtered.length > 1 && (
              <tfoot>
                <tr style={{ background: "var(--hf-surface-muted)", borderTop: "1px solid var(--hf-border)" }}>
                  <td colSpan={4} style={{ padding: "10px 16px", fontSize: 12, color: "var(--hf-text-muted)", fontWeight: 600 }}>{filtered.length} bills</td>
                  <td style={{ padding: "10px 16px", fontWeight: 800, color: "var(--hf-text)" }}>{fmtR(filtered.reduce((s, b) => s + Number(b.totalAmount), 0))}</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            )}
      ...
      {selected && !showApprove && !showCancel && !showPayModal && !showPopUpload && !showAttachUpload && !showEdit && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "flex-end", zIndex: 1000 }}>
          <div style={{ background: "var(--hf-surface)", width: 500, height: "100%", overflowY: "auto", boxShadow: "-8px 0 40px rgba(0,0,0,0.15)", padding: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                <div style={{ fontWeight: 800, fontSize: 17, color: "var(--hf-text)", marginBottom: 5 }}>{selected.supplierName}</div>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--hf-text-faint)" }}>#{selected.billNumber}</span>
                </div>
              </div>
              <button onClick={() => { setSelected(null); setRemittanceNotice("") }} style={{ background: "var(--hf-surface-sunken)", border: "none", borderRadius: "50%", width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--hf-text-muted)" }}><X size={14} /></button>
            </div>
            ...
            {selected.status === "REJECTED" && selected.rejectionReason && (
              <div style={{ background: "var(--hf-danger-soft)", border: "1px solid var(--hf-danger-border)", borderRadius: 10, padding: "12px 14px", marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--hf-danger-text)", marginBottom: 3, textTransform: "uppercase" as const, letterSpacing: 0.4 }}>Rejection reason</div>
                <div style={{ fontSize: 13, color: "var(--hf-text-secondary)" }}>{selected.rejectionReason}</div>
              </div>
            )}

            {/* Amount hero */}
            <div style={{ background: "var(--hf-surface-muted)", borderRadius: 12, padding: "18px 20px", marginBottom: 20, textAlign: "center" as const, border: "1px solid var(--hf-border)" }}>
              <div style={{ fontSize: 32, fontWeight: 900, color: "var(--hf-text)", letterSpacing: "-0.03em" }}>{fmtR(selected.totalAmount)}</div>
              {selected.vatAmount > 0 && <div style={{ fontSize: 12, color: "var(--hf-text-muted)", marginTop: 4 }}>{fmtR(selected.amount)} excl. VAT + {fmtR(selected.vatAmount)} VAT</div>}
              <div style={{ fontSize: 13, color: "var(--hf-text-muted)", marginTop: 4 }}>{CAT_LABELS[selected.category] ?? selected.category} · Due {fmtDate(selected.dueDate)}</div>
            </div>

            {/* Overdue alert */}
            {selected.overdue && (
              <div style={{ marginBottom: 14, padding: "10px 14px", background: "var(--hf-danger-soft)", border: "1px solid var(--hf-danger-border)", borderRadius: 9, display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--hf-danger-text)", fontWeight: 600 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 0, marginBottom: 20 }}>
              {[
                ...
                <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--hf-border-subtle)", fontSize: 13 }}>
                  <span style={{ color: "var(--hf-text-faint)", fontWeight: 600, minWidth: 110 }}>{k}</span>
            ...
            {selected.notes && (
              <div style={{ marginBottom: 14, padding: "12px 14px", background: "var(--hf-warning-soft)", borderRadius: 9, border: "1px solid var(--hf-warning-border)", fontSize: 13, color: "var(--hf-text-secondary)" }}>
            ...
            {["PAID"].includes(selected.status) && (
              <div style={{ marginBottom: 20 }}>
                <button onClick={() => { setError(""); setRemittanceNotice(""); sendRemittance.mutate() }}
                  ...
                  style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px", border: "1.5px solid var(--hf-violet-border)", borderRadius: 9, background: "var(--hf-violet-soft)", fontSize: 12, cursor: "pointer", color: "var(--hf-violet-text)", fontWeight: 600 }}>
                  <Mail size={13} /> {sendRemittance.isPending ? "Sending..." : "Email remittance advice to supplier"}
                </button>
                {remittanceNotice && (
                  <div style={{ marginTop: 8, padding: "7px 12px", background: "var(--hf-success-soft)", border: "1px solid var(--hf-success-border)", borderRadius: 7, fontSize: 12, color: "var(--hf-success-text-strong)" }}>{remittanceNotice}</div>
                )}
              </div>
            )}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {selected.status === "DRAFT" && <>
                ...
                <button onClick={() => setShowApprove(true)}
                  style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px", background: "var(--hf-success-soft-strong)", color: "var(--hf-success-text-strong)", border: "1px solid var(--hf-success-border)", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  <CheckCircle size={14} /> Approve bill
                </button>
              </>}
              {selected.status === "SECOND_APPROVAL" && (
                <button onClick={() => setShowApprove(true)}
                  style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px", background: "var(--hf-violet-soft)", color: "var(--hf-violet-text)", border: "1.5px solid var(--hf-violet-border)", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  <CheckCircle size={14} /> Give second approval
                </button>
              )}
              {["APPROVED","OVERDUE"].includes(selected.status) && !selected.batchId && (
                <button onClick={() => setShowPayModal(true)}
                  style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px", background: "var(--hf-info-soft)", color: "var(--hf-info-text)", border: "1.5px solid var(--hf-info-border)", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  <DollarSign size={14} /> Mark as paid
                </button>
              )}
              {selected.batchId && (
                <div style={{ width: "100%", padding: "10px 14px", background: "var(--hf-violet-soft)", border: "1px solid var(--hf-violet-border)", borderRadius: 9, fontSize: 13, color: "var(--hf-violet-text)", fontWeight: 600 }}>
                  Included in EFT batch — payment handled via batch
                </div>
              )}
              {!["PAID","CANCELLED"].includes(selected.status) && (
                <button onClick={() => setShowCancel(true)}
                  style={{ padding: "10px 14px", background: "var(--hf-surface-muted)", color: "var(--hf-text-faint)", border: "1.5px solid var(--hf-border)", borderRadius: 9, fontSize: 13, cursor: "pointer" }}>
                  <XCircle size={14} />
                </button>
              )}
            </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, padding: 20, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 620, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 25px 80px rgba(0,0,0,0.25)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 22 }}>
              ...
              <button onClick={() => { setShowCreate(false); setShowEdit(false); setError("") }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--hf-text-faint)", display: "flex" }}><X size={20} /></button>
            </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, backdropFilter: "blur(2px)" }}>
          <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.22)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 18 }}>
              ...
              <button onClick={() => setShowPayModal(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--hf-text-faint)", display: "flex" }}><X size={18} /></button>
            </div>
            <div style={{ padding: "14px", background: "var(--hf-surface-muted)", borderRadius: 9, marginBottom: 16, textAlign: "center" as const }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: "var(--hf-text)" }}>{fmtR(selected.totalAmount)}</div>
              <div style={{ fontSize: 12, color: "var(--hf-text-muted)" }}>{selected.supplierName} · #{selected.billNumber}</div>
            </div>
            <div>
            ...
            {bankAccounts.length > 0 && (
              <div style={{ marginTop: 12, padding: "8px 12px", background: "var(--hf-info-soft)", borderRadius: 8, fontSize: 12, color: "var(--hf-info-text)" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              ...
              <button onClick={() => payBill.mutate()} disabled={payBill.isPending}
                style={{ ...btnP, background: "var(--hf-success-solid-strong)", opacity: payBill.isPending ? 0.6 : 1 }}>
