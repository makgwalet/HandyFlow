...
export default function ChartOfAccountsTab() {
  ...
  if (isLoading) return <div style={{ padding: 60, textAlign: "center", color: "var(--hf-text-faint)" }}>Loading chart of accounts...</div>
  if (isError)   return <div style={{ padding: 60, textAlign: "center", color: "var(--hf-danger-text)" }}>Failed to load accounts</div>

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--hf-text)", margin: 0 }}>Chart of Accounts</h2>
                ...
                <tbody>
                  {accs.map((acc, i) => (
                    <tr key={acc.id}
                      ...
                      <td style={{ padding: "10px 16px" }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--hf-text)" }}>{acc.accountName}</span>
                        {acc.description && (
                          <p style={{ fontSize: 11, color: "var(--hf-text-faint)", margin: "1px 0 0" }}>{acc.description}</p>
                        )}
                      </td>
                      <td style={{ padding: "10px 16px" }}>
                        <span style={{ fontSize: 11, background: "var(--hf-surface-sunken)", color: "var(--hf-text-tertiary)",
                          padding: "2px 8px", borderRadius: 10, fontWeight: 500 }}>
                          {acc.accountSubtype}
                        </span>
                      </td>
                      <td style={{ padding: "10px 16px", fontSize: 13, fontWeight: 600,
                        color: acc.openingBalance > 0 ? "var(--hf-success-text-strong)" : "var(--hf-text)" }}>
      ...
      {showCreate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex",
          ...
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 440, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ fontSize: 12, color: "var(--hf-text-faint)", margin: "0 0 16px" }}>
              The 47 seeded accounts cover most cases — this is for the one or two a real business always ends up needing.
            </p>
            <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Code *</label>
                <input value={form.accountCode} onChange={e => setForm(f => ({ ...f, accountCode: e.target.value }))}
                  placeholder="5250" style={inp} />
              </div>
              <div style={{ flex: 2 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Name *</label>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Type *</label>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Subtype (optional)</label>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "var(--hf-text-secondary)", marginBottom: 5 }}>Description (optional)</label>
