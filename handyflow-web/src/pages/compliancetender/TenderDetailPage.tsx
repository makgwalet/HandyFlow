...
const STATUS_CFG: Record<string, { color: string; bg: string; label: string }> = {
  ...
  SUBMITTED: { color: "#0369A1", bg: "#EFF6FF", label: "Submitted" },
  CLARIFICATION: { color: "#0369A1", bg: "#EFF6FF", label: "Clarification" },
...
export default function TenderDetailPage() {
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 20px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Briefcase size={22} color="#0369A1" />
          </div>
          <div>
        ...
        <button onClick={() => downloadPdf(tender.id, tender.tenderNumber)}
          style={{ display: "flex", alignItems: "center", gap: 6, background: "#fff", color: "#0369A1", border: "1px solid #E2E8F0", borderRadius: 8, padding: "9px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      <Section title="Requirement Matrix" icon={<ClipboardCheck size={15} color="#0369A1" />}>
      ...
      <Section title="Key Personnel" icon={<Users size={15} color="#0369A1" />}>
        {personnel.length === 0 ? (
        ...
        {canManage && (
          <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
            ...
            <button onClick={() => pickedEmployee && personnelRole.trim() && addPersonnel.mutate()}
              ...
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "9px 14px", background: (!pickedEmployee || !personnelRole.trim()) ? "#CBD5E1" : "#0369A1", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: (!pickedEmployee || !personnelRole.trim()) ? "not-allowed" : "pointer" }}>
              <Plus size={13} /> Add
            </button>
          </div>
        )}
