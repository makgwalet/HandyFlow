      ...
      {!scheduledSessions || scheduledSessions.content.length === 0 ? (
        ...
        <div style={{ border: "1px solid var(--hf-border)", borderRadius: 12, overflow: "hidden" }}>
          {scheduledSessions.content.map((s, i) => (
            <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderTop: i === 0 ? "none" : "1px solid var(--hf-border-subtle)" }}>
              ...
              <span style={{ fontSize: 12, color: "var(--hf-text-muted)" }}>{s.startDate} → {s.endDate}{s.clientName ? ` · ${s.clientName}` : ""} · {s.enrolledCount} enrolled</span>
            </div>
          ))}
        </div>
      )}
