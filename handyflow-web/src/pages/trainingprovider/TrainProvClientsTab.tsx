    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
      <div style={{ background: "var(--hf-surface)", borderRadius: 16, padding: 28, width: 480, maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: "var(--hf-text)", margin: 0 }}>{initial ? "Edit client" : "Add a client"}</p>
        ...
        {save.isError && <p style={{ color: "var(--hf-danger-text)", fontSize: 12, marginTop: 12 }}>{(save.error as any)?.response?.data?.message ?? "Could not save this client"}</p>}

        <button onClick={() => save.mutate()} disabled={!form.tradingName || save.isPending}
          style={{ marginTop: 18, width: "100%", padding: "11px", borderRadius: 8, border: "none", background: TRAINPROV_ACCENT, color: "var(--hf-text-on-solid)", fontSize: 13.5, fontWeight: 700, cursor: "pointer", opacity: (!form.tradingName || save.isPending) ? 0.6 : 1 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: "var(--hf-text-faint)", margin: 0 }}>{clients.length} client{clients.length === 1 ? "" : "s"}</p>
      ...
      {isLoading ? (
        ...
        <p style={{ color: "var(--hf-text-faint)", fontSize: 13 }}>No clients yet.</p>
      ) : (
              ...
              <button onClick={() => setSelectedId(c.id)} style={{ display: "flex", alignItems: "center", gap: 10, background: "none", border: "none", cursor: "pointer", textAlign: "left", flex: 1 }}>
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <p style={{ fontSize: 13.5, fontWeight: 700, color: "var(--hf-text)", margin: 0 }}>{c.tradingName}</p>
                    <span style={{ fontSize: 10, fontWeight: 700, color: "var(--hf-text-faint)" }}>{c.clientCode}</span>
                    {c.status === "INACTIVE" && (
                      <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: "var(--hf-surface-sunken)", color: "var(--hf-text-muted)" }}>INACTIVE</span>
                    )}
                  </div>
                  <p style={{ fontSize: 11.5, color: "var(--hf-text-faint)", margin: 0 }}>{c.contactEmail ?? "No contact email"}</p>
                </div>
              </button>
