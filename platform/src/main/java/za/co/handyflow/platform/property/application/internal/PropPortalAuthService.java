...
/**
 * Direct mirror of WhsePortalAuthService/CollAgencyPortalAuthService —
 * shared.PortalUser/PortalJwtService/PortalUserRepository are genuinely
 * shared, portal-type-agnostic identity infrastructure; what a logged-in
 * user can actually see is entirely governed by PropPortalAccessGrant,
 * never by this class. Deliberately has no acceptAdditionalInvite() in
 * this first pass, same call made for every sibling module.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class PropPortalAuthService {
    ...
    public PortalAuthResponse registerViaInvite(String inviteToken, String password, String fullName) {
        PropPortalAccessGrant grant = grantRepo.findByInviteToken(inviteToken)
        ...
        log.info("[Property] Portal user registered via invite: {}", user.getEmail());
    ...
    public PortalAuthResponse login(String email, String password) {
        ...
        String token = portalJwtService.generateToken(user.getId(), user.getEmail());
        log.info("[Property] Portal login: {}", user.getEmail());
        return new PortalAuthResponse(token, user.getId(), user.getEmail(), user.getFullName());
    }
}
