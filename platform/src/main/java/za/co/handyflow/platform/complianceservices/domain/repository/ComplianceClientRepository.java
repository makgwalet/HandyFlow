...
public interface ComplianceClientRepository extends JpaRepository<ComplianceClient, UUID> {
    ...
    @Query("SELECT c FROM ComplianceClient c WHERE c.tenantId = :#{#tenantId.value} AND c.id = :id")
    Optional<ComplianceClient> findByIdForTenant(@Param("tenantId") TenantId tenantId, @Param("id") UUID id);

    @Query("SELECT c FROM ComplianceClient c WHERE c.tenantId = :#{#tenantId.value} " +
    ...
    @Query("SELECT c FROM ComplianceClient c WHERE c.tenantId = :#{#tenantId.value} AND c.crmCustomerId = :crmCustomerId")
