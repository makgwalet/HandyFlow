...
/**
 * The tender document builder — "Tender Summary" PDF, generated from the
 * tender's current LIVE state (requirement matrix + referenced personnel,
 * both resolved fresh at generation time). Deliberately NOT built from a
 * frozen {@code TenderSubmissionSnapshot} — this is a working document a
 * consultant prints or shares while a tender is still being prepared, not
 * the audit record of what was actually submitted. A
 * snapshot-to-PDF variant (rendering exactly what a specific submission
 * contained, unaffected by later changes) is a natural, separate future
 * addition once there's a real need for it — not built speculatively here.
 * <p>
 * Follows this platform's established iText 7 conventions exactly (see
 * {@code .claude/skills/pdf-generation/SKILL.md}, authored earlier this
 * session): {@code InvoicePdfService}'s section structure and brand
 * palette, the confirmed {@code logoUrl} data-URI gotcha, and
 * {@code TenantNumberingFacade}-issued document numbers (the tender
 * already has one — {@link Tender#getTenderNumber()} — no separate PDF
 * numbering needed).
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class TenderPdfService {
    ...
    public byte[] generateTenderSummaryPdf(TenantId tenantId, UUID tenderId) {
        ...
        List<TenderRequirement> requirements = requirementRepository.findByTender(tenantId, tenderId);
        List<TenderPersonnel> personnel = personnelRepository.findByTender(tenantId, tenderId);
        ...
        try {
            ...
            addHeader(doc, tender, tenant, regular, bold);
            ...
            doc.close();
            log.info("Tender summary PDF generated tender={} requirements={} personnel={} tenant={}",
                    tenderId, requirements.size(), personnel.size(), tenantId);
            return baos.toByteArray();
        } catch (Exception e) {
            throw new RuntimeException("Failed to generate tender summary PDF: " + e.getMessage(), e);
        }
    }
    ...
    private void addHeader(Document doc, Tender tender, TenantDetails tenant, PdfFont regular, PdfFont bold) {
    ...
    private void addRequirementMatrix(Document doc, List<TenderRequirement> requirements, PdfFont regular, PdfFont bold) {
        ...
        for (TenderRequirement r : requirements) {
    ...
    private void addPersonnel(Document doc, TenantId tenantId, List<TenderPersonnel> personnel, PdfFont regular, PdfFont bold) {
    ...
    private void addFooter(Document doc, PdfFont regular) {
        ...
        doc.add(new Paragraph(
                "This is a working document reflecting the tender's current status at the time of generation. "
                        + "It is a document/compliance readiness indicator, not a legal guarantee of tender eligibility.")
