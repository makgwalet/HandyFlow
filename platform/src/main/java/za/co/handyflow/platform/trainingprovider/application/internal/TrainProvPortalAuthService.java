...
/**
 * Deliberately named TrainProvPortalAuthService, not a generic
 * "PortalAuthService" — handles accepting invites TO THIS MODULE
 * specifically, using the shared PortalUser identity underneath.
 * Confirmed identical in shape to RecruitmentAgencyPortalAuthService /
 * BookingAgencyPortalAuthService / PayrollBureauPortalAuthService by
 * direct source read — this is the same real pattern, not guessed.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class TrainProvPortalAuthService {
    ...
    public PortalAuthResponse registerViaInvite(String inviteToken, String password, String fullName) {
        TrainProvPortalAccessGrant grant = grantRepo.findByInviteToken(inviteToken)
        ...
        log.info("Training provider portal user registered via invite: {}", user.getEmail());
    ...
    public PortalAuthResponse login(String email, String password) {
        ...
        String token = portalJwtService.generateToken(user.getId(), user.getEmail());
        log.info("Training provider portal login: {}", user.getEmail());
        return new PortalAuthResponse(token, user.getId(), user.getEmail(), user.getFullName());
    }
}
