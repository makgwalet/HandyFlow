...
public class ContractParty {
    ...
    // FIX: removed `signingToken` field (was @Column(name = "signing_token")) —
    // it duplicated the raw token already stored (correctly, alongside its
    // SHA-256 hash) on ContractSigningToken, as a second, unsynchronized copy
    // with none of that entity's expiry/revocation/used tracking. See
    // ContractingService.notifyNextParty() for where this is now sourced from
    // instead: signingTokenRepo.findActiveByPartyId(...). signingTokenSentAt
    // (an audit timestamp, not a secret) is unaffected and stays as-is.
    ...
    // FIX: setSigningToken(String) removed along with the field it wrote to.
