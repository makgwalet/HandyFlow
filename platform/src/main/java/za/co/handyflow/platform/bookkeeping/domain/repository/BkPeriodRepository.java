...
public interface BkPeriodRepository extends JpaRepository<BkPeriod, UUID> {
    ...
    @Query("SELECT p FROM BkPeriod p WHERE p.tenantId.value = :#{#tenantId.value} AND p.id = :id")
    Optional<BkPeriod> findActiveById(@Param("tenantId") TenantId tenantId, @Param("id") UUID id);

    @Query("SELECT p FROM BkPeriod p WHERE p.tenantId.value = :#{#tenantId.value} AND p.clientId = :clientId ORDER BY p.periodYear DESC, p.periodMonth DESC")
    ...
    @Query("SELECT p FROM BkPeriod p WHERE p.tenantId.value = :#{#tenantId.value} AND p.clientId = :clientId " +
