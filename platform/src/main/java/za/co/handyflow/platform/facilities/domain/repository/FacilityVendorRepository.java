...
public interface FacilityVendorRepository extends JpaRepository<FacilityVendor, UUID> {
    ...
    @Query("SELECT v FROM FacilityVendor v WHERE v.tenantId.value = :#{#tenantId.value} AND v.id = :id AND v.deletedAt IS NULL")
    Optional<FacilityVendor> findActiveById(@Param("tenantId") TenantId tenantId, @Param("id") UUID id);

    @Query("SELECT v FROM FacilityVendor v WHERE v.tenantId.value = :#{#tenantId.value} AND v.deletedAt IS NULL")
