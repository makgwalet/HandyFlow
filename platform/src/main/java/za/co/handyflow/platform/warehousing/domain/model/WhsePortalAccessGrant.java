...
/**
 * Grants a client's own staff access to a portal view of their inventory
 * and orders with this operator — direct analog of
 * CollAgencyPortalAccessGrant/PayPortalAccessGrant/RecPortalAccessGrant.
 * Its own dedicated table, same established precedent (each provider
 * module gets its own grant table, never a shared cross-module one).
 */
@Entity(name = "WhsePortalAccessGrant")
@Table(name = "whse_portal_access_grants")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
public class WhsePortalAccessGrant {
    ...
    public static WhsePortalAccessGrant createInvite(UUID tenantId, UUID clientId, String inviteEmail,
                                                      UUID invitedBy) {
        WhsePortalAccessGrant g = new WhsePortalAccessGrant();
        g.tenantId = tenantId;
        g.clientId = clientId;
