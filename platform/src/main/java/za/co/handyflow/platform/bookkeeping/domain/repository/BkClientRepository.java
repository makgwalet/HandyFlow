...
public interface BkClientRepository extends JpaRepository<BkClient, UUID> {
    ...
    @Query("SELECT c FROM BkClient c WHERE c.tenantId.value = :#{#tenantId.value} AND c.id = :id AND c.deletedAt IS NULL")
    Optional<BkClient> findActiveById(@Param("tenantId") TenantId tenantId, @Param("id") UUID id);

    @Query("SELECT c FROM BkClient c WHERE c.tenantId.value = :#{#tenantId.value} AND c.deletedAt IS NULL")
