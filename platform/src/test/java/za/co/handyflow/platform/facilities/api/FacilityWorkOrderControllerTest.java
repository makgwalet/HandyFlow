...
import za.co.handyflow.platform.facilities.dto.WorkOrderResponse;
...
@WebMvcTest(FacilityWorkOrderController.class)
@Import(WebMvcTestSecuritySupport.class)
class FacilityWorkOrderControllerTest {

    @Autowired MockMvc mvc;
    @Autowired ObjectMapper mapper;

    @MockitoBean FacilityWorkOrderService workOrderService;
    @MockitoBean FacilityPdfService pdfService;
    @MockitoBean EvidenceFacade evidenceFacade;
    @MockitoBean FeatureGuard featureGuard;

    static final String BASE = "/api/v1/facilities/work-orders";

    private WorkOrderResponse testResponse() {
        return new WorkOrderResponse(UUID.randomUUID(), "WO-00001", UUID.randomUUID(), null, null,
                "CORRECTIVE", "NORMAL", "OPEN", "Leaking tap", "Jane", null, null, null, null,
                LocalDate.now(), null, null, null, null, java.time.Instant.now());
    }

    @Test
    @WithMockUser(authorities = "FACILITIES_READ")
    @DisplayName("POST work-orders with only FACILITIES_READ returns 403")
    void createWithReadOnlyReturns403() throws Exception {
        mvc.perform(post(BASE).with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                            {"siteId":"%s","category":"CORRECTIVE","description":"Leaking tap"}
    ...
    @DisplayName("POST work-orders with FACILITIES_MANAGE returns 201")
    ...
    @DisplayName("GET work-orders returns 403 without any FACILITIES_* authority")
    ...
    @Test
    @WithMockUser(authorities = "FACILITIES_READ")
    @DisplayName("GET work-orders/{id} returns 200 with FACILITIES_READ")
    void getReturns200WithRead() throws Exception {
        WorkOrderResponse resp = testResponse();
